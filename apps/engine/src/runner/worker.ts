...
export async function runTask(redis: Redis, command: unknown): Promise<'success' | 'failed'> {
  ...
  await writer.emit({ type: 'log', level: 'info', message: `任务开始 ${cmd.request.method} ${cmd.request.url}` });
  ...
  try {
    ...
    const asserts = evaluateAsserts(cmd.asserts, { status: result.status, bodyText: result.bodyText });
    ...
    await postCallback(cmd.taskId, { outcome, failureKind: failureKind ?? undefined, message: failureKind ? 'ASSERT_FAILED' : '', lastSeq: writer.lastSeq });
    return outcome;
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    await writer.emit({ type: 'log', level: 'error', message: `网络/配置错误：${message}` });
    await writer.emit({ type: 'task-final', outcome: 'failed', failureKind: 'NETWORK_ERROR', message });
    await postCallback(cmd.taskId, { outcome: 'failed', failureKind: 'NETWORK_ERROR', message, lastSeq: writer.lastSeq });
    return 'failed';
  }
}
