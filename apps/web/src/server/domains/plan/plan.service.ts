...
import { DomainError, ErrCode, planPassRate, planUpsertSchema, planExecSchema } from '@rabbit/shared';
...
export async function listPlans(projectId: string, query: { keyword?: string; archived?: string; page?: number; pageSize?: number }) {
  ...
  return {
    ...
    items: plans.map((p) => {
      ...
      return {
        id: p.id, name: p.name, description: p.description, moduleId: p.moduleId,
        startAt: p.startAt?.toISOString() ?? null, endAt: p.endAt?.toISOString() ?? null,
        tags: (p.tags ?? []) as string[], settings: (p.settings ?? {}) as Record<string, unknown>,
        status: p.status, archivedAt: p.archivedAt?.toISOString() ?? null,
        caseCount: p.caseRefs.length,
        executed: stats.executed,
        progress: p.caseRefs.length === 0 ? 0 : Math.round((stats.executed / p.caseRefs.length) * 100),
...
export async function createPlan(projectId: string, userId: string, input: PlanUpsert) {
  return prisma.testPlan.create({
    data: {
      ...
      tags: input.tags,
      settings: input.settings ?? { allowDuplicate: false, autoUpdateStatus: false, threshold: 100 },
      createdBy: userId,
    },
...
export async function getPlan(projectId: string, planId: string) {
  ...
  const caseRows = caseIds.length
    ? await prisma.functionalCase.findMany({ where: { id: { in: caseIds }, deletedAt: null }, select: { id: true, num: true, name: true, level: true, steps: true, tags: true } })
    : [];
  ...
  const settings = (p.settings ?? {}) as { threshold?: number; allowDuplicate?: boolean; autoUpdateStatus?: boolean };
  return {
    id: p.id, name: p.name, description: p.description, moduleId: p.moduleId,
    startAt: p.startAt?.toISOString() ?? null, endAt: p.endAt?.toISOString() ?? null,
    ...
    cases: refs.map((r) => {
      ...
      return {
      refId: r.id, caseId: r.refId, num: c.num, name: c.name, level: c.level,
      tags: (c.tags ?? []) as string[],
      steps: (c.steps ?? []) as { desc: string; expect: string }[],
      execUserId: r.execUserId,
      status: r.status,
      result: (r.result ?? {}) as { actualResult?: string; steps?: { status: string; result: string }[]; comment?: string },
      execHistory: (r.execHistory ?? []) as { ts: string; userId: string; from: string; to: string }[],
      };
    }),
  };
}
...
export async function addPlanCases(projectId: string, planId: string, caseIds: string[], execUserId?: string) {
  const p = await loadPlan(projectId, planId);
  requireNotArchived(p.archivedAt);
  ...
  if (dup.length > 0 && !settings.allowDuplicate) {
    throw new DomainError(ErrCode.DUP_ASSOC, `重复关联 ${dup.length} 条用例（计划未开启「允许重复关联」）`);
  }
  for (const c of cases) {
    if (existingSet.has(c.id)) continue; // 开关开启时静默跳过
    await prisma.planCaseRef.create({
      data: { planId, refType: 'functional_case', refId: c.id, execUserId: execUserId ?? null, status: 'NOT_RUN' },
    });
  }
...
export async function execPlanCase(projectId: string, planId: string, refId: string, userId: string, input: PlanExec) {
  const p = await loadPlan(projectId, planId);
  requireNotArchived(p.archivedAt);
  const ref = await prisma.planCaseRef.findFirst({ where: { id: refId, planId } });
  if (!ref || ref.refType !== 'functional_case') throw new DomainError(ErrCode.PLAN_NOT_FOUND, '关联记录不存在');
  const caseRow = await prisma.functionalCase.findFirst({ where: { id: ref.refId, deletedAt: null }, select: { steps: true } });
  ...
  if (input.steps && input.steps.length > 0 && input.steps.length !== caseSteps.length) {
    throw new DomainError(ErrCode.VALIDATION_FAILED, `步骤结果数量（${input.steps.length}）须与用例步骤数（${caseSteps.length}）一致`);
  }
  const steps = input.steps ?? caseSteps.map(() => ({ status: input.status === 'NOT_RUN' ? 'NOT_RUN' : input.status, result: '' }));
  const history = (ref.execHistory ?? []) as { ts: string; userId: string; from: string; to: string }[];
  history.push({ ts: new Date().toISOString(), userId, from: ref.status, to: input.status });
  await prisma.planCaseRef.update({
    where: { id: refId },
    data: {
      status: input.status,
      result: JSON.parse(JSON.stringify({ actualResult: input.actualResult, steps, comment: input.comment })) as object,
...
export async function batchExecutor(projectId: string, planId: string, refIds: string[], execUserId: string) {
  const p = await loadPlan(projectId, planId);
  requireNotArchived(p.archivedAt);
  const r = await prisma.planCaseRef.updateMany({ where: { id: { in: refIds }, planId }, data: { execUserId } });
...
async function refreshPlanStatus(planId: string) {
  ...
  const plan = await prisma.testPlan.findFirst({ where: { id: planId }, select: { status: true, archivedAt: true } });
  ...
  if (next !== plan.status) await prisma.testPlan.update({ where: { id: planId }, data: { status: next } });
}
...
export async function getPlanReport(projectId: string, planId: string) {
  ...
  if (!report) {
    ...
    const created = await prisma.report.create({
      data: { taskId: `plan-${planId}`, projectId, planId, reportType: 'plan', name: `${plan.name}-报告`, createdBy: 'system' },
      select: { id: true, summary: true, createdAt: true },
    });
...
export async function updatePlanReportSummary(projectId: string, planId: string, summary: string) {
  const p = await loadPlan(projectId, planId);
  const existing = await prisma.report.findFirst({ where: { planId, reportType: 'plan' }, select: { id: true } });
  if (existing) {
    await prisma.report.update({ where: { id: existing.id }, data: { summary } });
    return { reportId: existing.id };
  }
  const created = await prisma.report.create({
    data: { taskId: `plan-${planId}`, projectId, planId, reportType: 'plan', name: `${p.name}-报告`, summary, createdBy: 'system' },
    select: { id: true },
  });
