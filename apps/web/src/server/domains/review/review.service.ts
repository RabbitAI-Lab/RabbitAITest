...
export async function listReviews(
  projectId: string,
  viewerId: string,
  query: { view?: string; keyword?: string; page?: number; pageSize?: number },
) {
...
export async function getReview(projectId: string, reviewId: string, viewerId: string) {
  ...
  const cases = await prisma.reviewCase.findMany({
    ...
    include: {
      case: {
        select: { id: true, num: true, name: true, level: true, precondition: true, steps: true },
      },
    },
  });
  return {
    id: r.id,
    name: r.name,
    reviewMode: r.reviewMode as "SINGLE" | "MULTI",
    status: r.status,
    reviewers: (r.reviewers ?? []) as string[],
    startAt: r.startAt?.toISOString() ?? null,
    endAt: r.endAt?.toISOString() ?? null,
    ...
    cases: cases.map((rc) => ({
      ...
      result: rc.result,
      reSubmit: rc.reSubmit,
      results: (rc.results ?? []) as {
        userId: string;
        result: string;
        comment: string;
        ts: string;
...
export async function createReview(projectId: string, userId: string, input: ReviewUpsert) {
  const r = await prisma.caseReview.create({
    data: {
      ...
      reviewers: toJson(input.reviewers),
...
export async function addReviewCases(projectId: string, reviewId: string, caseIds: string[]) {
  ...
  const existing = await prisma.reviewCase.findMany({
    where: { reviewId, caseId: { in: caseIds } },
    select: { caseId: true },
  });
...
export async function judgeReviewCase(
  ...
  const rc = await prisma.reviewCase.findFirst({
    where: { reviewId, caseId },
    select: { id: true, results: true },
  });
  if (!rc) throw new DomainError(ErrCode.CASE_NOT_FOUND, "该用例不在评审中");
  const results = (
    (rc.results ?? []) as { userId: string; result: string; comment: string; ts: string }[]
...
export async function batchReviewer(
  projectId: string,
  reviewId: string,
  caseIds: string[],
  reviewer: string,
  ...
  if (!reviewers.includes(reviewer))
    throw new DomainError(ErrCode.VALIDATION_FAILED, "目标用户不是该评审的评审人");
  ...
  for (const review of underway) {
    await prisma.reviewCase.updateMany({
      ...
      data: {
        ...
        results: [],
        reSubmit: true,
        reviewers: toJson(review.reviewers ?? []),
      },
    });
  }
}
