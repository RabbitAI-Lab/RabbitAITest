...
export interface OverviewRange { from: Date; to: Date }
...
export async function overview(projectId: string, orgId: string, range: OverviewRange) {
  const [caseTotal, caseNew, reviewPassRate, planProgress, bugPending, bugNew] = await Promise.all([
    ...
    prisma.functionalCase.count({ where: { projectId, deletedAt: null, createdAt: { gte: range.from, lte: range.to } } }),
    ...
    prisma.bug.count({ where: { projectId, deletedAt: null, createdAt: { gte: range.from, lte: range.to } } }),
  ]);
  return {
...
async function reviewPassRateOf(projectId: string) {
  ...
  return { passRate: judged === 0 ? null : Math.round((pass / judged) * 100), underway: reviews.filter((r) => r.status === 'UNDERWAY').length };
}
...
export async function todo(projectId: string, orgId: string, userId: string, kind: string, page: number, pageSize: number) {
  if (kind === 'review') {
    const rows = await prisma.reviewCase.findMany({
      where: {
        result: 'PENDING',
        review: { projectId, status: 'UNDERWAY', deletedAt: null, reviewers: { array_contains: userId } },
      },
    ...
    const caseRows = caseIds2.length
      ? await prisma.functionalCase.findMany({ where: { id: { in: caseIds2 }, deletedAt: null }, select: { id: true, name: true } })
      : [];
    const caseNameMap = new Map(caseRows.map((c) => [c.id, c.name]));
    return pageOf(rows.map((r) => ({
      id: r.id, kind: 'review' as const,
      title: caseNameMap.get(r.caseId) ?? '(已删除)', context: r.review.name,
  ...
  if (kind === 'exec') {
    ...
    const execCases = execCaseIds.length
      ? await prisma.functionalCase.findMany({ where: { id: { in: execCaseIds }, deletedAt: null }, select: { id: true, name: true } })
      : [];
    const execCaseMap = new Map(execCases.map((c) => [c.id, c.name]));
    return pageOf(rows.map((r) => ({
      id: r.id, kind: 'exec' as const,
      title: execCaseMap.get(r.refId) ?? '(已删除)', context: r.plan.name,
  ...
  return pageOf(rows.map((b) => ({
    id: b.id, kind: 'bug' as const,
    title: b.title, context: `B-${String(b.num).padStart(4, '0')}`,
    href: `/bugs/${b.id}`,
...
export async function followed(userId: string, page: number, pageSize: number) {
  ...
  for (const f of follows) {
    if (f.entityType === 'functional_case') {
      const c = await prisma.functionalCase.findFirst({ where: { id: f.entityId, deletedAt: null }, select: { id: true, name: true, updatedAt: true } });
      if (c) items.push({ id: c.id, kind: 'case', title: c.name, href: `/cases/${c.id}`, updatedAt: c.updatedAt.toISOString() });
    } else if (f.entityType === 'bug') {
      const b = await prisma.bug.findFirst({ where: { id: f.entityId, deletedAt: null }, select: { id: true, title: true, updatedAt: true } });
      if (b) items.push({ id: b.id, kind: 'bug', title: b.title, href: `/bugs/${b.id}`, updatedAt: b.updatedAt.toISOString() });
    } else if (f.entityType === 'test_plan') {
      const p = await prisma.testPlan.findFirst({ where: { id: f.entityId, deletedAt: null }, select: { id: true, name: true, updatedAt: true } });
      if (p) items.push({ id: p.id, kind: 'plan', title: p.name, href: `/plans/${p.id}`, updatedAt: p.updatedAt.toISOString() });
    } else if (f.entityType === 'case_review') {
      const r = await prisma.caseReview.findFirst({ where: { id: f.entityId, deletedAt: null }, select: { id: true, name: true, updatedAt: true } });
      if (r) items.push({ id: r.id, kind: 'review', title: r.name, href: `/reviews/${r.id}`, updatedAt: r.updatedAt.toISOString() });
    }
  }
...
export async function created(projectId: string, kind: string, page: number, pageSize: number) {
  if (kind === 'review') {
    const rows = await prisma.caseReview.findMany({ where: { projectId, deletedAt: null }, orderBy: { createdAt: 'desc' } });
    return pageOf(rows.map((r) => ({ id: r.id, kind: 'review', title: r.name, href: `/reviews/${r.id}`, createdAt: r.createdAt.toISOString() })), page, pageSize);
  }
  if (kind === 'plan') {
    const rows = await prisma.testPlan.findMany({ where: { projectId, deletedAt: null }, orderBy: { createdAt: 'desc' } });
    return pageOf(rows.map((p) => ({ id: p.id, kind: 'plan', title: p.name, href: `/plans/${p.id}`, createdAt: p.createdAt.toISOString() })), page, pageSize);
  }
  if (kind === 'bug') {
    const rows = await prisma.bug.findMany({ where: { projectId, deletedAt: null }, orderBy: { createdAt: 'desc' } });
    return pageOf(rows.map((b) => ({ id: b.id, kind: 'bug', title: b.title, href: `/bugs/${b.id}`, createdAt: b.createdAt.toISOString() })), page, pageSize);
  }
  const rows = await prisma.functionalCase.findMany({ where: { projectId, deletedAt: null }, orderBy: { createdAt: 'desc' } });
  return pageOf(rows.map((c) => ({ id: c.id, kind: 'case', title: c.name, href: `/cases/${c.id}`, createdAt: c.createdAt.toISOString() })), page, pageSize);
}
