...
export async function deleteModule(projectId: string, scene: string, id: string) {
  ...
  const def = await prisma.moduleNode.findFirst({
    where: { projectId, scene, isDefault: true },
    select: { id: true },
  });
  if (!def) throw new DomainError(ErrCode.MODULE_NOT_FOUND, "默认模块缺失");
  await prisma.$transaction(async (tx) => {
    if (scene === "case")
      await tx.functionalCase.updateMany({
        where: { projectId, moduleId: { in: ids } },
        data: { moduleId: def.id },
      });
    if (scene === "bug")
...
export async function moveModule(
  ...
  if (parentId) {
    ...
    if (sub.includes(parentId))
      throw new DomainError(ErrCode.VALIDATION_FAILED, "不可移动到自己的后代节点");
    const parent = await prisma.moduleNode.findFirst({
      where: { id: parentId, projectId, scene },
      select: { id: true },
    });
