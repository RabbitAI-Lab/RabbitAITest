...
export async function parseExcel(
  buffer: Buffer,
): Promise<{ rows: ImportRow[]; ignoredColumns: string[] }> {
  ...
  for (let r = 2; r <= ws.rowCount; r++) {
    ...
    void dynByKey;
    const name = get("name");
    const stepLines = get("stepDesc")
      .split(/\r?\n/)
      .map((s) => s.trim())
      .filter(Boolean);
    const expectLines = get("expect")
      .split(/\r?\n/)
      .map((s) => s.trim());
...
function coerceDyn(field: DynField, raw: string): unknown {
  ...
  switch (field.type) {
    case "number":
      return Number(v);
    case "checkbox":
      return ["是", "true", "1", "yes"].includes(v.toLowerCase());
    case "multi_select":
      return v
        .split(/[,，;；]/)
        .map((s) => s.trim())
...
export async function importCases(
  ...
  const existing = await prisma.functionalCase.findMany({
    where: {
      ...
      deletedAt: null,
      ...(rows.some((r) => r.num)
        ? { num: { in: rows.map((r) => r.num).filter((n): n is number => Boolean(n)) } }
        : {}),
    },
  ...
  for (const row of rows) {
    ...
    else if (row.level === "INVALID")
      report.errors.push({ row: row.rowNo, reason: `非法等级：${row.level}` });
    else if (row.num !== undefined && !byNum.has(row.num) && row.num < 1)
      report.errors.push({ row: row.rowNo, reason: `非法编号：${row.num}` });
  }
  ...
  const resolveModule = async (path?: string): Promise<string> => {
    ...
    for (const seg of segs) {
      ...
      let node: { id: string } | null = await prisma.moduleNode.findFirst({
        where: { projectId, scene: "case", name: seg, parentId: parent },
        select: { id: true },
      });
      if (!node) {
        node = await prisma.moduleNode.create({
          data: { projectId, scene: "case", name: seg, parentId: parent },
        });
      }
      parent = node.id;
      last = node.id;
  ...
  const validator = buildValidator(
    defs as unknown as import("@rabbit/shared").FieldDefInput[],
    (template?.fields as import("@rabbit/shared").TemplateFieldBinding[] | undefined) ?? undefined,
  );
  for (const row of rows) {
    ...
    if (!parsedFields.success) {
      report.errors.push({
        row: row.rowNo,
        reason: `动态字段校验失败：${parsedFields.error.issues[0]?.message ?? ""}`,
      });
    }
  }
  if (report.errors.length > 0) {
      ...
      if (existId) {
        await tx.functionalCase.update({
          ...
          data: {
            ...
            steps: toJson(row.steps),
            level: row.level,
            tags: toJson(row.tags),
            moduleId,
            fields: toJson(row.fields),
            version: { increment: 1 },
          },
        });
        await tx.changeLog.create({
          data: {
            ...
            entityId: existId,
            seq: (await tx.functionalCase.findUnique({
              where: { id: existId },
              select: { version: true },
            }))!.version,
            action: "import_overwrite",
            userId,
            diff: toJson({ source: "excel" }),
          },
        });
...
async function defaultModuleId(projectId: string): Promise<string> {
  const def = await prisma.moduleNode.findFirst({
...
interface XmindTopic {
  title?: string;
  notes?: { plain?: { content?: string } } | string;
  labels?: string[];
...
export async function parseXmind(
  ...
  if (!contentKey)
    throw new DomainError(
      ErrCode.VALIDATION_FAILED,
      "仅支持 Xmind v2.x（content.json）；旧版 XML 格式请先升级导出",
    );
...
export async function buildTemplate(
  ...
  ws.addRow([
    ...
    "冒烟,登录",
    ...defs.map((d) =>
      d.type === "single_select" || d.type === "radio"
        ? ((d as unknown as { options?: { options?: string[] } }).options?.options?.join("/") ?? "")
        : "",
    ),
  ]);
  ...
  return {
    ...
    filename: `${project?.name ?? "项目"}-用例导入模板.xlsx`,
  };
}
...
export async function exportCases(
  ...
  if (opts.format === "xmind") {
    ...
    const caseTopic = (c: (typeof selected)[number]): XmindTopic => ({
      ...
      children: {
        attached: (want("steps") ? c.steps : []).map((s) => ({
          title: s.desc,
    ...
    const content = [
      {
        ...
        class: "sheet",
        title: "画布",
        rootTopic: { id: "root", title: project?.name ?? "用例", children: { attached: roots } },
      },
    ];
    const zip = zipSync({
      "content.json": strToU8(JSON.stringify(content)),
      "metadata.json": strToU8(
        JSON.stringify({ creator: { name: "RabbitAITest", version: "1.0" } }),
      ),
    });
    return {
      buffer: Buffer.from(zip),
      filename: `${project?.name ?? "项目"}-用例-${stamp}.xmind`,
  ...
  return {
    ...
    filename: `${project?.name ?? "项目"}-用例-${stamp}.xlsx`,
    contentType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
  };
}
