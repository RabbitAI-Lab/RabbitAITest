...
export async function listDependencies(projectId: string, caseId: string) {
  const c = await prisma.functionalCase.findFirst({ where: { id: caseId, projectId, deletedAt: null }, select: { id: true } });
  ...
  const ids = [...new Set([...preRefs.map((d) => d.preCaseId), ...postRefs.map((d) => d.postCaseId)])];
  const cases = ids.length
    ? await prisma.functionalCase.findMany({ where: { id: { in: ids }, deletedAt: null }, select: { id: true, num: true, name: true, level: true } })
    : [];
  const caseMap = new Map(cases.map((c) => [c.id, c]));
  return {
    pre: preRefs.filter((d) => caseMap.has(d.preCaseId)).map((d) => ({ id: d.id, case: caseMap.get(d.preCaseId)! })),
    post: postRefs.filter((d) => caseMap.has(d.postCaseId)).map((d) => ({ id: d.id, case: caseMap.get(d.postCaseId)! })),
  };
}
...
export async function addDependency(projectId: string, preCaseId: string, postCaseId: string) {
  ...
  const dup = await prisma.caseDependency.findFirst({ where: { preCaseId, postCaseId }, select: { id: true } });
...
export async function caseReviews(projectId: string, caseId: string) {
  const rows = await prisma.reviewCase.findMany({
    ...
    orderBy: { id: 'desc' },
    include: {
      review: { select: { id: true, name: true, status: true, reviewMode: true, startAt: true, endAt: true } },
    },
  });
  return rows.map((r) => ({
    reviewId: r.review.id, name: r.review.name, status: r.review.status,
    result: r.result, reSubmit: r.reSubmit,
...
export async function casePlans(projectId: string, caseId: string, viewerId: string) {
  ...
  return refs.map((r) => ({
    planId: r.plan.id, name: r.plan.name, planStatus: r.plan.status,
...
export async function caseBugs(projectId: string, caseId: string) {
  const refs = await prisma.bugCaseRef.findMany({ where: { refType: 'functional_case', refId: caseId } });
  const bugIds = refs.map((r) => r.bugId);
  const bugs = bugIds.length
    ? await prisma.bug.findMany({ where: { id: { in: bugIds }, projectId, deletedAt: null }, select: { id: true, num: true, title: true, status: true } })
    : [];
...
export async function linkCaseBug(projectId: string, caseId: string, bugId: string) {
  const [c, b] = await Promise.all([
    prisma.functionalCase.findFirst({ where: { id: caseId, projectId, deletedAt: null }, select: { id: true } }),
    prisma.bug.findFirst({ where: { id: bugId, projectId, deletedAt: null }, select: { id: true } }),
  ]);
  if (!c) throw new DomainError(ErrCode.CASE_NOT_FOUND, '用例不存在或已删除');
...
export async function unlinkCaseBug(projectId: string, caseId: string, bugId: string) {
  const r = await prisma.bugCaseRef.deleteMany({ where: { bugId, refType: 'functional_case', refId: caseId } });
...
export async function listComments(entityType: string, entityId: string) {
  ...
  return rows.map((c) => ({
    ...
    userId: c.userId, userName: c.user.name,
    createdAt: c.createdAt.toISOString(), updatedAt: c.updatedAt.toISOString(),
  }));
}

export async function addComment(userId: string, entityType: string, entityId: string, content: string, parentId?: string) {
...
export async function updateComment(userId: string, canModerate: boolean, commentId: string, content: string) {
  const c = await prisma.comment.findFirst({ where: { id: commentId, deletedAt: null }, select: { id: true, userId: true } });
  if (!c) throw new DomainError(ErrCode.VALIDATION_FAILED, '评论不存在');
  if (c.userId !== userId && !canModerate) throw new DomainError(ErrCode.FORBIDDEN, '仅作者或项目管理员可操作该评论');
...
export async function listChanges(entityType: string, entityId: string) {
  ...
  return rows.map((r) => ({
    id: r.id, seq: r.seq, action: r.action, diff: r.diff,
...
export async function nextSeq(tx: { changeLog: { count: (a: { where: { entityType: string; entityId: string } }) => Promise<number> } }, entityType: string, entityId: string): Promise<number> {
