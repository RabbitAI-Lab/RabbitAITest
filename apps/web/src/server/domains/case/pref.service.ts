...
export interface CaseView { id: string; name: string; query: Record<string, unknown>; columns?: string[]; isDefault: boolean }
...
export async function createView(userId: string, projectId: string, input: Omit<CaseView, 'id'>) {
  ...
  const view: CaseView = { id: `v_${Date.now().toString(36)}${Math.random().toString(36).slice(2, 6)}`, ...input };
  const next = input.isDefault ? views.map((v) => ({ ...v, isDefault: false })) : views;
  await saveViews(userId, projectId, [...next, view]);
  return view;
}

export async function updateView(userId: string, projectId: string, id: string, input: Omit<CaseView, 'id'>) {
  const views = await loadViews(userId, projectId);
...
export async function putPreference(userId: string, key: string, projectId: string, value: unknown) {
  await prisma.userPreference.upsert({
