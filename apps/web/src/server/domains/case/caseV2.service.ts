...
async function validateFields(
  ...
  if (!parsed.success) {
    ...
    throw new DomainError(
      ...
      first ? `自定义字段校验失败：${first.path.join(".")} ${first.message}` : "自定义字段校验失败",
    );
  }
...
export async function createCaseV2(
  projectId: string,
  orgId: string,
  userId: string,
  input: {
  ...
  const { templateId, fields } = await validateFields(
    orgId,
    projectId,
    "case",
    input.fields ?? {},
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.changeLog.create({
      data: {
        ...
        action: "create",
        userId,
        diff: toJson({ after: { name: input.name, level: input.level } }),
      },
    });
...
export async function updateCaseV2(
  projectId: string,
  orgId: string,
  userId: string,
  caseId: string,
  ...
  const { templateId, fields } = await validateFields(
    ...
    "case",
    input.fields ?? {},
    input.templateId ?? existing.templateId ?? undefined,
  );
  ...
  for (const f of ["name", "level", "steps", "tags"] as const) {
    if (JSON.stringify(before[f]) !== JSON.stringify(after[f]))
      diff[f] = { before: before[f], after: after[f] };
  }
  if (before.moduleId !== after.moduleId)
    diff.module = { before: before.moduleId, after: after.moduleId };
  ...
  for (const k of dynKeys) {
    if (JSON.stringify(before.fields[k]) !== JSON.stringify(after.fields[k]))
...
export async function listCasesV2(projectId: string, q: CaseListQueryV2, viewerId: string) {
  ...
  if (q.fields) {
    ...
    if (dynFilters.length)
      where.AND = [...(Array.isArray(where.AND) ? where.AND : []), ...dynFilters];
  }
  if (q.followedBy) {
...
export async function setFollow(projectId: string, userId: string, caseId: string, on: boolean) {
  ...
  if (on) {
    await prisma.follow.upsert({
      where: {
        userId_entityType_entityId: { userId, entityType: "functional_case", entityId: caseId },
      },
      update: {},
    ...
    await prisma.follow.deleteMany({
      where: { userId, entityType: "functional_case", entityId: caseId },
    });
  }
  return { following: on };
}
...
export async function listFollowedCaseIds(userId: string): Promise<string[]> {
  const follows = await prisma.follow.findMany({
    where: { userId, entityType: "functional_case" },
    select: { entityId: true },
  });
...
export async function batchCases(
  projectId: string,
  orgId: string,
  userId: string,
  action: "move" | "copy" | "delete" | "update",
  input: CaseBatchInput,
) {
  ...
  switch (action) {
    case "move": {
      ...
      const m = await prisma.moduleNode.findFirst({
        where: { id: input.moduleId, projectId, scene: "case" },
        select: { id: true },
      });
      if (!m) throw new DomainError(ErrCode.MODULE_NOT_FOUND, "目标模块不存在");
    ...
    case "update": {
      ...
      if (input.addTags?.length || input.removeTags?.length) {
        for (const c of existing) {
          ...
          await prisma.functionalCase.update({
            where: { id: c.id },
            data: { ...(input.level ? { level: input.level } : {}), tags: [...tags] },
          });
        }
        return { affected: existing.length };
      }
      if (Object.keys(data).length === 0)
        throw new DomainError(ErrCode.VALIDATION_FAILED, "缺少批量编辑内容");
