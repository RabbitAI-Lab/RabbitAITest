...
export async function listCases(projectId: string, q: CaseListQuery) {
  ...
  const [total, items] = await Promise.all([
    ...
    prisma.functionalCase.findMany({
      where,
      orderBy: q.orderBy === "num" ? { num: q.order } : { updatedAt: q.order },
...
export async function updateCase(
  ...
  caseId: string,
  userId: string,
  input: CaseCreateInput & { version: number },
) {
  return prisma.$transaction(async (tx) => {
...
export async function softDeleteCase(projectId: string, caseId: string, userId: string) {
  ...
  const seq =
    (await prisma.changeLog.count({ where: { entityType: "functional_case", entityId: caseId } })) +
    1;
