...
export async function createUser(actorId: string, input: UserCreateInput) {
  ...
  if (active >= USER_LIMIT) throw new DomainError(ErrCode.USER_TOO_MANY, `社区版用户上限 ${USER_LIMIT}`);
  // 邮箱唯一校验含软删用户（删除用户不清理占用，与基线口径一致）
  const exist = await prisma.user.findFirst({ where: { email: input.email }, select: { id: true } });
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'system', action: 'user.create', objectType: 'user', objectId: user.id },
  }).catch(() => undefined);
  // 初始密码仅创建响应一次性返回
  return { ...user, initialPassword: password };
}

export async function updateUser(actorId: string, userId: string, input: { name?: string; phone?: string | null }) {
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'system', action: 'user.update', objectType: 'user', objectId: userId },
  }).catch(() => undefined);
  return updated;
}
...
export async function resetPassword(actorId: string, userId: string) {
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'system', action: 'user.reset_password', objectType: 'user', objectId: userId },
  }).catch(() => undefined);
  return { newPassword: password }; // 一次性返回
}

/** 禁用用户：立即失效其全部会话（守卫按请求校验用户状态，SYS-004 §2）。 */
export async function setUserStatus(actorId: string, userId: string, status: 'ACTIVE' | 'DISABLED') {
  const existing = await prisma.user.findFirst({ where: { id: userId, deletedAt: null }, select: { id: true, email: true } });
  if (!existing) throw new DomainError(ErrCode.USER_NOT_FOUND, '用户不存在');
  if (existing.email === 'admin@rabbit.test') throw new DomainError(ErrCode.VALIDATION_FAILED, '内置管理员不可禁用');
  await prisma.user.update({ where: { id: userId }, data: { status } });
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'system', action: `user.${status.toLowerCase()}`, objectType: 'user', objectId: userId },
  }).catch(() => undefined);
...
export async function deleteUser(actorId: string, userId: string) {
  const existing = await prisma.user.findFirst({ where: { id: userId, deletedAt: null }, select: { id: true, email: true } });
  if (!existing) throw new DomainError(ErrCode.USER_NOT_FOUND, '用户不存在');
  if (existing.email === 'admin@rabbit.test') throw new DomainError(ErrCode.VALIDATION_FAILED, '内置管理员不可删除');
  await prisma.user.update({ where: { id: userId }, data: { deletedAt: new Date(), status: 'DISABLED' } });
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'system', action: 'user.delete', objectType: 'user', objectId: userId },
  }).catch(() => undefined);
}
