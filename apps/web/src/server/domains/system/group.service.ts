...
export async function createGroup(actorId: string, ref: ScopeRef, input: GroupUpsertInput) {
  if (ref.scope === "org" && !ref.orgId)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "缺少组织上下文");
  if (ref.scope === "project" && !ref.projectId)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "缺少项目上下文");
...
async function loadGroup(id: string) {
  const g = await prisma.group.findFirst({
    where: { id },
    select: { id: true, name: true, isSystem: true, scope: true, orgId: true, projectId: true },
  });
...
export async function deleteGroup(id: string) {
  ...
  if (count > 0)
    throw new DomainError(ErrCode.VALIDATION_FAILED, `组内仍有 ${count} 名成员，请先移出`);
