...
export async function listOrgMembers(orgId: string, q: { keyword?: string; page: number; pageSize: number }) {
  const where = {
    orgId,
    user: {
      deletedAt: null,
  ...
  const [total, items] = await Promise.all([
    ...
    prisma.orgMember.findMany({
      ...
      take: q.pageSize,
      select: { userId: true, createdAt: true, user: { select: { id: true, email: true, name: true, phone: true } } },
    }),
  ]);
...
export async function listOrgProjects(orgId: string, query: { deleted?: string; keyword?: string }) {
  const where = {
    orgId,
  ...
  const projects = await prisma.project.findMany({
    ...
    orderBy: { createdAt: 'desc' },
    select: {
      id: true, name: true, num: true, description: true, status: true, deletedAt: true, purgeAt: true,
      createdAt: true, modules: true, _count: { select: { members: true } },
    },
  });
  return {
    total: projects.length,
    items: projects.map((p) => ({
      id: p.id, name: p.name, num: p.num, description: p.description, status: p.status,
      modules: p.modules, memberCount: p._count.members,
...
export async function createOrgProject(orgId: string, actorId: string, input: { name: string; description?: string }) {
  return prisma.$transaction(async (tx) => {
    await tx.$executeRawUnsafe('SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))', 'projects', orgId);
    ...
    await tx.projectMember.create({ data: { projectId: project.id, userId: actorId, role: 'OWNER' } });
...
export async function getProjectInfo(projectId: string) {
  const p = await prisma.project.findFirst({
    ...
    select: { id: true, name: true, num: true, description: true, modules: true, status: true, createdAt: true, org: { select: { id: true, name: true } } },
  });
  if (!p) throw new DomainError(ErrCode.PROJECT_NOT_FOUND, '项目不存在或无权访问');
  return { ...p, modules: p.modules as Record<string, boolean>, createdAt: p.createdAt.toISOString() };
}

export async function updateProject(projectId: string, actorId: string, input: ProjectUpdateInput) {
  const existing = await prisma.project.findFirst({ where: { id: projectId, deletedAt: null }, select: { id: true, status: true } });
  if (!existing) throw new DomainError(ErrCode.PROJECT_NOT_FOUND, '项目不存在或无权访问');
  if (existing.status === 'ENDED') throw new DomainError(ErrCode.PROJECT_ENDED, '项目已结束，禁止修改');
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'project', projectId, action: 'project.update', objectType: 'project', objectId: projectId },
  }).catch(() => undefined);
...
export async function setProjectEnded(projectId: string, actorId: string, ended: boolean) {
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'project', projectId, action: ended ? 'project.close' : 'project.reopen', objectType: 'project', objectId: projectId },
  }).catch(() => undefined);
...
export async function softDeleteProject(projectId: string, actorId: string) {
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'project', projectId, action: 'project.delete', objectType: 'project', objectId: projectId },
  }).catch(() => undefined);
}
...
export async function restoreProject(projectId: string, actorId: string) {
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'project', projectId, action: 'project.restore', objectType: 'project', objectId: projectId },
  }).catch(() => undefined);
  return { id: projectId };
}
...
export async function purgeProject(projectId: string): Promise<void> {
  await prisma.$transaction(async (tx) => {
    const caseIds = (await tx.functionalCase.findMany({ where: { projectId }, select: { id: true } })).map((c) => c.id);
    const bugIds = (await tx.bug.findMany({ where: { projectId }, select: { id: true } })).map((b) => b.id);
    const reviewIds = (await tx.caseReview.findMany({ where: { projectId }, select: { id: true } })).map((r) => r.id);
    const planIds = (await tx.testPlan.findMany({ where: { projectId }, select: { id: true } })).map((p) => p.id);
    const execTaskIds = (await tx.execTask.findMany({ where: { projectId }, select: { id: true } })).map((t) => t.id);
    const entityIds = [...caseIds.map((id) => `functional_case:${id}`), ...bugIds.map((id) => `bug:${id}`), ...planIds.map((id) => `test_plan:${id}`)];
    // 横切
    if (entityIds.length) {
    ...
    // case 域
    await tx.caseDependency.deleteMany({ where: { OR: [{ preCaseId: { in: caseIds } }, { postCaseId: { in: caseIds } }] } });
...
export async function listProjectMembers(projectId: string, q: { keyword?: string; page: number; pageSize: number }) {
  const where = {
    projectId,
    user: {
      deletedAt: null,
  ...
  const [total, items] = await Promise.all([
    ...
    prisma.projectMember.findMany({
      ...
      skip: (q.page - 1) * q.pageSize,
      take: q.pageSize,
      select: { userId: true, role: true, createdAt: true, user: { select: { id: true, email: true, name: true } } },
    }),
  ]);
  // 成员所在项目组（展示用）
  ...
  return {
    ...
    items: items.map((m) => ({
      ...
      groups: gms.filter((g) => g.userId === m.userId).map((g) => ({ name: g.group.name, isSystem: g.group.isSystem })),
    })),
  };
}
...
export async function addProjectMembers(projectId: string, actorId: string, userIds: string[]) {
  ...
  // 组织成员校验（从组织用户搜索添加）
  const orgMembers = await prisma.orgMember.findMany({ where: { orgId: org.orgId, userId: { in: userIds } }, select: { userId: true } });
  const orgUserIds = new Set(orgMembers.map((m) => m.userId));
  const notInOrg = userIds.filter((id) => !orgUserIds.has(id));
  if (notInOrg.length) throw new DomainError(ErrCode.VALIDATION_FAILED, '仅可添加组织成员（先由系统管理员创建用户）');
  const memberGroup = await prisma.group.findFirst({ where: { scope: 'project', projectId, name: '项目成员' }, select: { id: true } });
  let added = 0;
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'project', projectId, action: 'project.members.add', objectType: 'project', objectId: projectId, detail: { count: users.length } },
  }).catch(() => undefined);
  return { added };
}
...
export async function removeProjectMember(projectId: string, actorId: string, userId: string) {
  ...
  await prisma.auditLog.create({
    data: { userId: actorId, scope: 'project', projectId, action: 'project.members.remove', objectType: 'user', objectId: userId },
  }).catch(() => undefined);
