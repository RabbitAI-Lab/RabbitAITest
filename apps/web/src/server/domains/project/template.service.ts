...
export async function listFieldDefs(orgId: string, scene?: string) {
  ...
  return defs.map((d) => ({
    ...
    isSystem: d.isSystem,
    enabled: !("enabled" in (d.options as object))
      ? true
      : Boolean((d.options as { enabled?: boolean }).enabled),
    createdAt: d.createdAt.toISOString(),
  }));
}

export async function createFieldDef(orgId: string, input: FieldDefUpsertInput) {
  const dup = await prisma.fieldDef.findFirst({
    where: { orgId, scene: input.scene, key: input.key },
    select: { id: true },
  });
...
export async function updateFieldDef(orgId: string, id: string, input: FieldDefUpsertInput) {
  const existing = await prisma.fieldDef.findFirst({
    where: { id, orgId },
    select: { id: true, type: true, key: true },
  });
  if (!existing) throw new DomainError(ErrCode.VALIDATION_FAILED, "字段不存在");
  if (existing.type !== input.type)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "字段类型不可变更（停用后新建）");
  if (existing.key !== input.key)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "字段标识不可变更");
...
export async function deleteFieldDef(orgId: string, id: string) {
  const existing = await prisma.fieldDef.findFirst({
    where: { id, orgId },
    select: { id: true, key: true, scene: true, options: true },
  });
  if (!existing) throw new DomainError(ErrCode.VALIDATION_FAILED, "字段不存在");
...
export async function listTemplates(orgId: string, projectId: string | null, scene?: string) {
  ...
  const caseCounts = ids.length
    ? await prisma.functionalCase.groupBy({
        by: ["templateId"],
        where: { templateId: { in: ids } },
        _count: { _all: true },
      })
  ...
  for (const r of [...bugCounts, ...caseCounts]) {
    if (r.templateId)
      countMap.set(
        r.templateId,
        (countMap.get(r.templateId) ?? 0) + (r._count as { _all: number })._all,
      );
  }
...
export async function createTemplate(
  ...
  if (input.scene === "bug") {
    ...
    if (count >= BUG_TEMPLATE_LIMIT)
      throw new DomainError(ErrCode.VALIDATION_FAILED, `缺陷模板上限 ${BUG_TEMPLATE_LIMIT} 个`);
  }
  const t = await prisma.template.create({
    data: {
      orgId,
      projectId: projectMode ? projectId : null,
      scene: input.scene,
      name: input.name,
      fields: input.fields,
    },
...
export async function updateTemplateFields(
  orgId: string,
  id: string,
  fields: TemplateUpsertInput["fields"],
) {
  ...
  const defs = await prisma.fieldDef.findMany({
    where: { orgId, scene: existing.scene },
    select: { key: true },
  });
  const known = new Set(defs.map((d) => d.key));
  const unknown = fields.filter((f) => !known.has(f.fieldKey)).map((f) => f.fieldKey);
  if (unknown.length)
    throw new DomainError(ErrCode.VALIDATION_FAILED, `未定义的字段：${unknown.join(", ")}`);
...
export async function setDefaultTemplate(orgId: string, projectId: string | null, id: string) {
  const t = await prisma.template.findFirst({
    where: { id, orgId },
    select: { id: true, scene: true, projectId: true },
  });
  if (!t) throw new DomainError(ErrCode.TEMPLATE_NOT_FOUND, "模板不存在");
  await prisma.$transaction(async (tx) => {
    await tx.template.updateMany({
      where: { orgId, scene: t.scene, projectId: t.projectId, isDefault: true },
...
export async function copyTemplate(orgId: string, projectId: string | null, id: string) {
  const t = await prisma.template.findFirst({
    where: { id, orgId },
    select: { id: true, name: true, scene: true, fields: true, isDefault: true },
  });
  if (!t) throw new DomainError(ErrCode.TEMPLATE_NOT_FOUND, "模板不存在");
  if (t.scene === "bug") {
    const where = await effectiveWhere(orgId, projectId, "bug");
...
export async function deleteTemplate(orgId: string, projectId: string | null, id: string) {
  const t = await prisma.template.findFirst({
    where: { id, orgId },
    select: { id: true, isDefault: true, isSystem: true, scene: true },
  });
  if (!t) throw new DomainError(ErrCode.TEMPLATE_NOT_FOUND, "模板不存在");
  if (t.isDefault)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "默认模板不可删除（先切换默认）");
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (stateIds.length) {
      await tx.workflowTransition.deleteMany({
        where: { OR: [{ fromStateId: { in: stateIds } }, { toStateId: { in: stateIds } }] },
      });
...
export async function enableProjectTemplateMode(orgId: string, projectId: string, actorId: string) {
  ...
  if (already)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "项目模板已启用（不可逆，无法重复操作）");
  await prisma.$transaction(async (tx) => {
    const orgTemplates = await tx.template.findMany({
      where: { orgId, projectId: null },
      select: { id: true, scene: true, name: true, fields: true, isDefault: true },
    });
  ...
  return defs
    .map((d) => {
      ...
      return {
        ...
        required: d.required,
        defaultValue:
          (opts.defaultValue as string | number | string[] | boolean | null) ?? undefined,
        options: opts,
...
async function resolveWorkflowTemplate(orgId: string, projectId: string, templateId?: string) {
  ...
  if (!effective)
    throw new DomainError(ErrCode.TEMPLATE_NOT_FOUND, "默认缺陷模板缺失，请重新初始化");
  return effective.id;
}
...
export async function getWorkflow(orgId: string, projectId: string, templateId?: string) {
  ...
  const transitions = await prisma.workflowTransition.findMany({
    where: {
      OR: [
        { fromStateId: { in: states.map((s) => s.id) } },
        { toStateId: { in: states.map((s) => s.id) } },
      ],
    },
  });
...
export async function createWorkflowState(
  orgId: string,
  projectId: string,
  input: { serial: string; isStart?: boolean; isEnd?: boolean },
) {
  const tid = await resolveWorkflowTemplate(orgId, projectId);
  const dup = await prisma.workflowState.findFirst({
    where: { templateId: tid, serial: input.serial },
    select: { id: true },
  });
  ...
  return prisma.$transaction(async (tx) => {
    if (input.isStart) {
      await tx.workflowState.updateMany({
        where: { templateId: tid, isStart: true },
        data: { isStart: false },
      });
    }
    ...
    return tx.workflowState.create({
      data: {
        ...
        scene: "bug",
        serial: input.serial,
        isStart: input.isStart ?? startCount === 0,
...
export async function updateWorkflowState(
  ...
  const state = await prisma.workflowState.findFirst({
    where: { id: stateId },
    select: { id: true, templateId: true, serial: true, isStart: true },
  });
  if (!state) throw new DomainError(ErrCode.VALIDATION_FAILED, "状态不存在");
  const tid = await resolveWorkflowTemplate(orgId, projectId);
  if (state.templateId !== tid)
    throw new DomainError(ErrCode.VALIDATION_FAILED, "状态不属于当前生效模板");
  return prisma.$transaction(async (tx) => {
    if (input.isStart) {
      await tx.workflowState.updateMany({
        where: { templateId: tid, isStart: true },
...
export async function deleteWorkflowState(orgId: string, projectId: string, stateId: string) {
  const state = await prisma.workflowState.findFirst({
    where: { id: stateId },
    select: { id: true, isStart: true, templateId: true, serial: true },
  });
  if (!state) throw new DomainError(ErrCode.VALIDATION_FAILED, "状态不存在");
  ...
  await prisma.$transaction(async (tx) => {
    await tx.workflowTransition.deleteMany({
      where: { OR: [{ fromStateId: stateId }, { toStateId: stateId }] },
    });
...
export async function updateWorkflowTransitions(
  orgId: string,
  projectId: string,
  transitions: { fromSerial: string; toSerial: string }[],
) {
  const tid = await resolveWorkflowTemplate(orgId, projectId);
  const states = await prisma.workflowState.findMany({
    where: { templateId: tid },
  ...
  for (const t of transitions) {
    if (!bySerial.has(t.fromSerial) || !bySerial.has(t.toSerial)) {
      throw new DomainError(
        ...
        `流转含未知状态：${t.fromSerial} → ${t.toSerial}`,
      );
    }
    if (t.fromSerial === t.toSerial)
      throw new DomainError(ErrCode.VALIDATION_FAILED, "不允许自流转");
  }
  await prisma.$transaction(async (tx) => {
    await tx.workflowTransition.deleteMany({
      where: {
...
async function copyWorkflowTx(
  tx: Pick<Prisma.TransactionClient, "workflowState" | "workflowTransition">,
  ...
  for (const t of transitions) {
    ...
    if (from && to)
      await tx.workflowTransition.create({ data: { fromStateId: from, toStateId: to } });
  }
}
...
async function seedDefaultWorkflow(templateId: string) {
  const pending = await prisma.workflowState.create({
    data: { templateId, scene: "bug", serial: "待处理", isStart: true },
  });
  const processing = await prisma.workflowState.create({
    data: { templateId, scene: "bug", serial: "处理中" },
  });
  const closed = await prisma.workflowState.create({
    data: { templateId, scene: "bug", serial: "已关闭", isEnd: true },
  });
