    ...
    try {
      ...
      const ctx: ProjectCtx = {
        ...
        requirePerm(point: string) {
          if (!permissions.has(point)) throw new DomainError(ErrCode.FORBIDDEN, `缺少权限点 ${point}`);
        },
        requireWritable() {
          if (project.status === 'ENDED') throw new DomainError(ErrCode.PROJECT_ENDED, ErrMsg[ErrCode.PROJECT_ENDED]!);
        },
      };
...
export interface OrgCtx extends AuthedCtx { orgId: string; permissions: Set<string>; requirePerm(point: string): void }
