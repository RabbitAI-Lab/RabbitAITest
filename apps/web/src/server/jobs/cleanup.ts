...
/** 清理一轮（单测直接覆盖此纯逻辑入口）。 */
export async function runCleanupOnce(now = new Date()): Promise<{ changes: number; audits: number; purgedProjects: number }> {
  ...
  const due = await prisma.project.findMany({ where: { purgeAt: { lt: now } }, select: { id: true } });
  for (const p of due) {
    try {
...
export async function ensureCleanupScheduler(): Promise<void> {
  ...
  try {
    ...
    console.warn('[cleanup] Redis 不可用，定时清理未注册：', err instanceof Error ? err.message : err);
  }
}
