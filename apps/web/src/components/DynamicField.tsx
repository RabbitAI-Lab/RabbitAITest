...
export interface DynFieldDef extends FieldDefInput {
  id?: string;
}
...
export function DynamicFieldForm({
  ...
  const visible = defs.filter(
    (d) => d.enabled !== false && (!bound || bound.some((b) => b.fieldKey === d.key)),
  );
          ...
          <div key={def.key} className={renderKind(def.type) === "TextArea" ? "col-span-2" : ""}>
            ...
            {renderKind(def.type) === "MemberSelect" ? (
              <Select
                ...
                virtual={false}
                mode={
                  (def.options as { multiple?: boolean } | undefined)?.multiple
                    ? "multiple"
                    : undefined
                }
                showSearch
...
export function DynamicFieldCell({ def, value }: { def: DynFieldDef; value: unknown }) {
  if (value === undefined || value === null || value === "")
    return <span className="text-[#A8ABB0]">—</span>;
