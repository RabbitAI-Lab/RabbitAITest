...
export function ModuleTreePanel({
  projectId, scene, selectedId, includeChildren, onSelect, onIncludeChildrenChange, canEdit,
}: {
  projectId: string;
  scene: string;
  ...
  const create = useMutation({
    mutationFn: (body: { name: string; parentId?: string | null }) => moduleApi.create(projectId, scene, body),
    onSuccess: () => { invalidate(); message.success('模块已创建'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '创建失败'),
  });
  const rename = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) => moduleApi.rename(projectId, scene, id, name),
    onSuccess: () => { invalidate(); message.success('已重命名'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '重命名失败'),
  });
  const remove = useMutation({
    mutationFn: (id: string) => moduleApi.remove(projectId, scene, id),
    onSuccess: () => { invalidate(); onSelect?.(null); message.success('模块已删除，其下用例已移至默认模块'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '删除失败'),
  });
  const move = useMutation({
    mutationFn: ({ id, parentId, order }: { id: string; parentId: string | null; order: number }) => moduleApi.move(projectId, scene, id, { parentId, order }),
    onSuccess: () => invalidate(),
    onError: (e) => { invalidate(); message.error(e instanceof Error ? e.message : '移动失败'); },
  });
  ...
  const visibleRoots = (data?.items ?? []).filter((n) => !keyword || JSON.stringify(n).includes(keyword));

  const promptCreate = (parentId: string | null, isChild: boolean) => {
    let value = '';
    modal.confirm({
      title: isChild ? '新建子模块' : '新建同级模块',
      content: <Input placeholder="模块名称" onChange={(e) => { value = e.target.value; }} />,
  ...
  const promptRename = (node: ModuleNodeDto) => {
    ...
    modal.confirm({
      title: '重命名模块',
      content: <Input defaultValue={node.name} onChange={(e) => { value = e.target.value; }} />,
  ...
  return (
    <div className="w-[240px] shrink-0 bg-white border border-[#E5E6EB] rounded-md flex flex-col" data-testid={`module-panel-${scene}`}>
      <div className="flex items-center gap-1 px-2 py-2 border-b border-[#F0F1F3]">
        ...
        {canEdit && (
          <>
            <Button type="text" size="small" icon={<Plus size={14} />} onClick={() => promptCreate(null, false)} data-testid="module-add-root" />
          </>
        )}
        <Button type="text" size="small" icon={<RefreshCw size={13} />} onClick={() => invalidate()} />
      </div>
      <div className="px-2 py-1.5">
        <Input size="small" allowClear placeholder="定位模块" prefix={<FoldVertical size={12} className="text-[#A8ABB0]" />} value={keyword} onChange={(e) => setKeyword(e.target.value)} />
      </div>
      <div className="flex-1 overflow-auto px-1 pb-2">
        <Tree
          ...
          onDrop={(info) => {
            ...
            const parentId = info.dropToGap ? (findNode(all, String(targetKey))?.parentId ?? null) : String(targetKey);
          ...
          titleRender={(node) => {
            ...
            return (
              <Dropdown
                ...
                menu={{
                  items: [
                    ...(canEdit ? [{ key: 'add-child', label: '新建子模块', onClick: () => promptCreate(String(node.key), true) }] : []),
                    ...(canEdit && n && !n.isDefault ? [{ key: 'rename', label: '重命名', onClick: () => n && promptRename(n) }] : []),
                    ...(canEdit && n && !n.isDefault ? [{ key: 'delete', label: '删除', danger: true }] : []),
                  ],
                  onClick: ({ key }) => {
                    if (key === 'delete' && n) {
                      modal.confirm({
      ...
      <div className="border-t border-[#F0F1F3] px-3 py-2 text-xs text-[#646A73] flex items-center justify-between">
        ...
        {onIncludeChildrenChange && (
          <label className="flex items-center gap-1 cursor-pointer" data-testid="include-children">
            <input type="checkbox" checked={Boolean(includeChildren)} onChange={(e) => onIncludeChildrenChange(e.target.checked)} />
            含子级
          </label>
        )}
      </div>
