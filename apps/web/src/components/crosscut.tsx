    ...
    <div key={c.id} className={isReply ? "ml-8 mt-2 border-l-2 border-[#F0F1F3] pl-3" : "mt-3"}>
      {editing?.id === c.id ? (
        ...
        <div className="flex items-start gap-2">
          <span className="w-6 h-6 rounded-full bg-[#574BFF]/10 text-[#574BFF] text-xs flex items-center justify-center shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p
              className="text-[13px] text-[#3D4350] whitespace-pre-wrap break-all"
    ...
    const walk = (obj: Record<string, unknown>, prefix = "") => {
      for (const [k, v] of Object.entries(obj)) {
        if (v && typeof v === "object" && "before" in (v as object) && "after" in (v as object)) {
          ...
          const fmt = (x: unknown) =>
            typeof x === "object" ? JSON.stringify(x)?.slice(0, 120) : String(x ?? "空");
          rows.push(
          ...
          <List.Item className="!px-0 relative">
            ...
            <div className="w-full">
              <p className="text-[13px]">
                ...
                <span className="text-[#A8ABB0] text-xs">
                  #{it.seq} · {it.createdAt.replace("T", " ").slice(0, 16)}
                </span>
              </p>
              {renderDiff(it.diff)}
            </div>
