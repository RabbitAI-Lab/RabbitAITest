...
import { Button, Drawer, Empty, Input, InputNumber, Modal, Popconfirm, Progress, Select, Switch, Table, Tabs, Tag, Tooltip, Tree } from 'antd';
import { Plus } from 'lucide-react';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { use, useEffect, useMemo, useState } from 'react';
import type { DataNode } from 'antd/es/tree';
import { bugApi, caseApiV2, memberApi, moduleApi, planApi, type PlanCaseRow } from '@rabbit/api-client';
...
export default function PlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const exec = useMutation({
    mutationFn: (p: { refId: string; body: Record<string, unknown> }) => planApi.exec(projectId!, id, p.refId, p.body),
    onSuccess: () => { invalidate(); message.success('执行结果已保存'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '操作失败'),
  });
  const batchExecutor = useMutation({
    mutationFn: (p: { refIds: string[]; execUserId: string }) => planApi.batchExecutor(projectId!, id, p.refIds, p.execUserId),
    onSuccess: (r) => { invalidate(); message.success(`已更新 ${r.affected} 条执行人`); setBatchOpen(false); setBatchUser(undefined); setSelectedRefs([]); },
    onError: (e) => message.error(e instanceof Error ? e.message : '操作失败'),
  });
  const removeCase = useMutation({
    mutationFn: (refId: string) => planApi.removeCase(projectId!, id, refId),
    onSuccess: () => { invalidate(); message.success('已取消关联'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '操作失败'),
  });
  const archive = useMutation({
    mutationFn: (toArchived: boolean) => planApi.archive(projectId!, id, toArchived),
    onSuccess: (_r, toArchived) => { invalidate(); message.success(toArchived ? '计划已归档（只读）' : '已取消归档'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '操作失败'),
  });
  const saveSettings = useMutation({
    mutationFn: (settings: { allowDuplicate: boolean; autoUpdateStatus: boolean; threshold: number }) =>
      planApi.update(projectId!, id, { settings }),
    onSuccess: () => { invalidate(); setSettingsOpen(false); message.success('设置已保存'); },
  ...
  const filtered = useMemo(
    () => cases.filter((c) =>
      (!statusFilter || c.status === statusFilter) && (!executorFilter || c.execUserId === executorFilter)),
  ...
  if (isLoading || !plan) return <div className="rabbit-card p-16 flex justify-center"><Empty description="加载中…" /></div>;

  const total = cases.length;
  const threshold = typeof plan.settings.threshold === 'number' ? plan.settings.threshold : 100;
  const statusMeta = STATUS_META[archived ? 'ARCHIVED' : plan.status] ?? { label: plan.status, color: 'default' };

  return (
    <div>
      {/* 头部 */}
      <div className="flex items-center gap-3 flex-wrap mb-4">
        <a href="/plans" className="text-[13px] text-[#87888D] hover:text-[#574BFF] no-underline">‹ 测试计划</a>
        <h1 className="text-lg font-medium m-0">{plan.name}</h1>
        <Tag color={statusMeta.color}>{statusMeta.label}</Tag>
        <span className="text-xs text-[#A8ABB0]">
          {plan.startAt ? plan.startAt.slice(0, 10) : '—'} ~ {plan.endAt ? plan.endAt.slice(0, 10) : '—'} · {total} 条用例
        </span>
        <div className="ml-auto flex items-center gap-2">
          <div className="flex items-center gap-2.5 border border-[#ECEEF1] rounded-md px-3 py-1.5 bg-white" data-testid="plan-pass-rate-box">
            <Progress
              type="circle"
              size={44}
            ...
            <div className="text-xs leading-4">
              <p className="m-0">通过率<span className="text-[#A8ABB0]">（已执行口径）</span></p>
              <p className="text-[#A8ABB0] m-0">
                阈值 {threshold}% ·{' '}
                {plan.thresholdMet === null ? '—' : plan.thresholdMet
                  ? <span className="text-[#52C41A]">达标</span>
                  : <span className="text-[#FF4D4F]">未达标</span>}
              </p>
            </div>
          </div>
          <Button onClick={() => setSettingsOpen(true)} disabled={!canUpdate}>更多设置</Button>
          {archived ? (
            canUpdate && <Button onClick={() => archive.mutate(false)} data-testid="btn-unarchive">取消归档</Button>
          ) : (
            canUpdate && (
              <Popconfirm
                title="归档后计划只读"
                ...
                okText="确认归档"
                okButtonProps={{ danger: true, ...({ 'data-testid': 'btn-archive-confirm' } as Record<string, unknown>) }}
                onConfirm={() => archive.mutate(true)}
              >
                <Button danger data-testid="btn-archive">归档</Button>
              </Popconfirm>
            )
          )}
        </div>
      ...
      {archived && (
        ...
        >
          ...
          {canUpdate && <Button size="small" onClick={() => archive.mutate(false)}>取消归档</Button>}
        </div>
      )}
      ...
      {tab === 'cases' ? (
        <div className="rabbit-card" data-testid="plan-cases-table">
          ...
          <div className="flex gap-2 items-center p-3 border-b border-[#F0F1F3] flex-wrap">
            <Button type="primary" icon={<Plus size={14} />} disabled={!writable} onClick={() => setLinkOpen(true)} data-testid="btn-link-cases">
              关联用例
            </Button>
            <Button
            ...
            <MemberSelect
              ...
              onChange={(v) => setExecutorFilter(Array.isArray(v) ? undefined : (v as string | undefined))}
              placeholder="执行人"
            />
            {stats && (
              <span className="ml-auto text-xs text-[#A8ABB0]">
                已执行 {stats.executed}/{total} · 通过 {stats.pass} · 失败 {stats.fail} · 阻塞 {stats.blocked} · 跳过 {stats.skipped} · 未执行 {stats.pending}
              </span>
            )}
          </div>
          <Table<PlanCaseRow>
            ...
            expandable={{
              ...
              expandedRowRender: (row) => (
                <StepExecPanel projectId={projectId} planId={id} row={row} disabled={!writable} onSaved={invalidate} />
              ),
            }}
            columns={[
              { title: '编号', dataIndex: 'num', width: 88, render: (n: number) => <span className="text-[#87888D]">{padNum(n)}</span> },
              {
                title: '用例名称', dataIndex: 'name', ellipsis: true,
                render: (v: string, row) => <a className="text-[#574BFF]" href={`/cases/${row.caseId}`}>{v}</a>,
              },
              { title: '等级', dataIndex: 'level', width: 64, render: (l: string) => <Tag color={levelColor[l] ?? 'default'}>{l}</Tag> },
              {
                title: '执行人', dataIndex: 'execUserId', width: 180,
              ...
              {
                title: '我的执行状态', dataIndex: 'status', width: 120,
                render: (v: string, row) => (
                  <Select
                    className="w-full"
                    value={v}
                    disabled={!writable}
                    onChange={(nv) => exec.mutate({
                      refId: row.refId,
                      body: { status: nv, actualResult: row.result.actualResult ?? '', comment: row.result.comment ?? '' },
                    })}
              ...
              {
                title: '实际结果', key: 'actual', width: 200,
                render: (_, row) => (
                ...
                render: (_, row) => (
                  <span className="flex gap-1 whitespace-nowrap">
                    <Button
                      ...
                      className="!px-0"
                      disabled={!writable}
                      onClick={() => setExpandedKeys((prev) => prev.includes(row.refId) ? prev.filter((k) => k !== row.refId) : [...prev, row.refId])}
                    ...
                    <Popconfirm
                      ...
                      disabled={!writable}
                    >
                      <Button type="link" size="small" danger className="!px-0" disabled={!writable}>取消关联</Button>
                    </Popconfirm>
                  </span>
                ),
              },
        ...
        <PlanReportTab projectId={projectId} planId={id} archived={archived} canUpdate={canUpdate} nameOf={nameOf} />
      )}
      ...
      <Modal
        ...
        onOk={() => batchUser && batchExecutor.mutate({ refIds: selectedRefs, execUserId: batchUser })}
      ...
      >
        <div className="py-2">
          <MemberSelect
            ...
            placeholder="选择执行人"
          />
          <p className="text-xs text-[#A8ABB0] mt-2 mb-0">将为已勾选的 {selectedRefs.length} 条用例统一指派执行人。</p>
        </div>
      </Modal>
...
/* ── Tab1：步骤级执行面板（展开行） ── */
function StepExecPanel({ projectId, planId, row, disabled, onSaved }: {
  projectId: string; planId: string; row: PlanCaseRow; disabled: boolean; onSaved: () => void;
}) {
  const { message } = useApp();
  const qc = useQueryClient();
  const initSteps = () =>
  ...
  const derive = (ss: { status: string }[]) =>
    ss.some((s) => s.status === 'FAIL') ? 'FAIL'
      : ss.some((s) => s.status === 'BLOCKED') ? 'BLOCKED'
        : ss.length > 0 && ss.every((s) => s.status === 'PASS') ? 'PASS' : 'NOT_RUN';
  ...
  const save = useMutation({
    mutationFn: () => planApi.exec(projectId, planId, row.refId, {
      status,
      actualResult: actual,
      comment: execComment,
  ...
  return (
    <div className="bg-[#F7F8FA] p-4 space-y-3" data-testid="step-exec-panel">
      ...
      {row.steps.length === 0 && <p className="text-[13px] text-[#A8ABB0] m-0">该用例无步骤，可直接标记整体状态后保存。</p>}
      {row.steps.map((s, i) => {
        const st = steps[i];
        ...
        return (
          ...
          >
            ...
            <Input
              ...
              value={st?.result ?? ''}
              disabled={disabled}
              onChange={(e) => setSteps((prev) => prev.map((x, idx) => (idx === i ? { ...x, result: e.target.value } : x)))}
      ...
      <div className="flex items-center gap-2 justify-end flex-wrap">
        <Select
          ...
          disabled={disabled}
          onChange={setStatus}
          options={Object.entries(EXEC_META).map(([v, m]) => ({ value: v, label: <span style={{ color: m.color }}>{m.label}</span> }))}
      ...
      {row.execHistory.length > 0 && (
        <p className="text-xs text-[#A8ABB0] m-0">
          执行历史：{row.execHistory.slice(-3).reverse().map((h) => `${h.ts.slice(5, 16).replace('T', ' ')} ${EXEC_META[h.to]?.label ?? h.to}`).join('；')}
        </p>
      )}
    </div>
  );
...
/* ── Tab2：报告 ── */
function PlanReportTab({ projectId, planId, archived, canUpdate, nameOf }: {
  projectId: string; planId: string; archived: boolean; canUpdate: boolean; nameOf: (uid: string) => string;
}) {
  const { message } = useApp();
  const qc = useQueryClient();
  ...
  useEffect(() => { if (report) setSummary(report.summary); }, [report]);
  const save = useMutation({
    mutationFn: () => planApi.saveSummary(projectId, planId, summary),
    onSuccess: () => { message.success('总结已保存'); void qc.invalidateQueries({ queryKey: ['plan-report', projectId, planId] }); },
    onError: (e) => message.error(e instanceof Error ? e.message : '保存失败'),
  });
  if (isLoading || !report) return <div className="rabbit-card p-16 flex justify-center"><Empty description="报告生成中…" /></div>;
    ...
    <div>
      <div className="grid grid-cols-4 gap-4 text-sm" data-testid="plan-report-cards">
        <div className="rabbit-card p-4">
          ...
          <p className="text-2xl font-semibold mt-1 mb-0" style={{ color: report.thresholdMet === false ? '#FF4D4F' : '#52C41A' }}>
            {report.passRate === null ? '—' : `${report.passRate}%`}{' '}
            <span className="text-xs">{report.thresholdMet === null ? '' : report.thresholdMet ? '达标' : '未达标'}</span>
          </p>
        </div>
        <div className="rabbit-card p-4">
          <p className="text-xs text-[#A8ABB0] m-0">已执行 / 总数</p>
          <p className="text-2xl font-semibold mt-1 mb-0">{s.executed} <span className="text-sm text-[#A8ABB0]">/ {report.cases.length}</span></p>
        </div>
        <div className="rabbit-card p-4">
          <p className="text-xs text-[#A8ABB0] m-0">通过 / 失败</p>
          <p className="text-2xl font-semibold mt-1 mb-0">
            <span className="text-[#52C41A]">{s.pass}</span> <span className="text-sm text-[#A8ABB0]">/</span> <span className="text-[#FF4D4F]">{s.fail}</span>
          </p>
        </div>
        <div className="rabbit-card p-4">
          <p className="text-xs text-[#A8ABB0] m-0">阻塞 / 跳过</p>
          <p className="text-2xl font-semibold mt-1 mb-0">
            <span className="text-[#FA8C16]">{s.blocked}</span> <span className="text-sm text-[#A8ABB0]">/</span> {s.skipped}
          </p>
        </div>
      </div>
      <div className="rabbit-card mt-4">
        <p className="rabbit-card-title">用例结果汇总 <span className="text-xs text-[#A8ABB0] font-normal">共 {report.cases.length} 条</span></p>
        <Table<PlanCaseRow>
          rowKey="refId"
          size="small"
          ...
          pagination={false}
          columns={[
            { title: '编号', dataIndex: 'num', width: 90, render: (n: number) => <span className="text-[#87888D]">{padNum(n)}</span> },
            { title: '用例名称', dataIndex: 'name', ellipsis: true },
            { title: '执行人', dataIndex: 'execUserId', width: 110, render: (v: string | null) => (v ? nameOf(v) : '—') },
            {
            ...
            {
              title: '实际结果摘要', key: 'actual', ellipsis: true,
              render: (_, row) => {
                const failed = (row.result.steps ?? []).findIndex((x) => x.status === 'FAIL');
                const text = row.result.actualResult
                  || (failed >= 0 ? `步骤 ${failed + 1}：${row.result.steps?.[failed]?.result || '失败'}` : '')
                  || '—';
                return <span className="text-xs text-[#87888D]">{text}</span>;
              },
            },
            {
              title: '执行时间', key: 'ts', width: 150,
              render: (_, row) => {
                const last = row.execHistory[row.execHistory.length - 1];
                return last
                  ? <span className="text-xs text-[#87888D]">{last.ts.replace('T', ' ').slice(5, 16)} {nameOf(last.userId)}</span>
...
/* ── 更多设置抽屉 ── */
function SettingsDrawer({ open, onClose, plan, saving, onSave }: {
  open: boolean;
  onClose: () => void;
  ...
  const [threshold, setThreshold] = useState(typeof plan.settings.threshold === 'number' ? plan.settings.threshold : 100);
  return (
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-[#A8ABB0] m-0">关闭时同一用例二次关联将被拒绝（422 code 10009 DUP_ASSOC）。</p>
          </div>
          <Switch checked={allowDuplicate} onChange={setAllowDuplicate} data-testid="drawer-switch-duplicate" />
        </div>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[13px] font-medium m-0">自动更新用例状态</p>
            <Tooltip title="接口域联动后生效（CASE-006），本迭代仅保存配置">
              <p className="text-xs text-[#A8ABB0] m-0 cursor-help">关联接口场景执行成功后更新功能用例状态 ⓘ</p>
            </Tooltip>
          </div>
          <Switch checked={autoUpdateStatus} onChange={setAutoUpdateStatus} data-testid="drawer-switch-auto-update" />
        </div>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[13px] font-medium m-0">通过阈值（%）</p>
            <p className="text-xs text-[#A8ABB0] m-0">通过率 ≥ 阈值 → 达标（通过率口径：pass/(pass+fail+blocked)，跳过不计分母）。</p>
          </div>
          <InputNumber min={0} max={100} value={threshold} onChange={(v) => setThreshold(v ?? 100)} data-testid="drawer-input-threshold" />
        </div>
        <Button type="primary" block loading={saving} onClick={() => onSave({ allowDuplicate, autoUpdateStatus, threshold })} data-testid="btn-save-plan-settings">
          保存设置
        </Button>
      </div>
    </Drawer>
...
function LinkCasesModal({ projectId, open, onClose, onConfirm }: {
  projectId: string; open: boolean; onClose: () => void; onConfirm: (caseIds: string[]) => Promise<void>;
}) {
  const { message } = useApp();
  ...
  return (
    <Modal title="关联用例" open={open} onCancel={onClose} footer={null} width={760} destroyOnHidden>
      <div className="flex gap-3 mt-1" data-testid="link-cases-modal">
        ...
        <div className="flex-1 min-w-0">
          ...
          />
          {isParent && <p className="text-xs text-[#FA8C16] mb-2">已选择父模块：按当前筛选列出其与全部子模块用例（含子级）。</p>}
          <Table
            rowKey="id"
            size="small"
            loading={isLoading}
...
/* ── 缺陷弹窗：新建（预填失败步骤）/ 关联已有 ── */
function BugModal({ projectId, row, onClose }: { projectId: string; row: PlanCaseRow; onClose: () => void }) {
  const { message } = useApp();
  ...
  const createAndLink = useMutation({
    ...
    onSuccess: (bug) => { message.success(`已创建 BUG-${String(bug.num).padStart(4, '0')} 并关联用例`); onClose(); },
  ...
  const linkExisting = useMutation({
    ...
    onSuccess: () => { message.success('已关联缺陷'); onClose(); },
    onError: (e) => message.error(e instanceof Error ? e.message : '关联失败'),
  });
    ...
    >
      ...
      {tab === 'new' ? (
        <div className="space-y-3" data-testid="new-bug-form">
          <div>
            <label className="block text-[13px] mb-1">标题 <span className="text-[#FF4D4F]">*</span></label>
            <Input value={title} maxLength={512} onChange={(e) => setTitle(e.target.value)} data-testid="input-bug-title" />
          </div>
          <div>
            <label className="block text-[13px] mb-1">描述（已预填失败步骤）</label>
            <Input.TextArea rows={6} maxLength={8000} value={desc} onChange={(e) => setDesc(e.target.value)} data-testid="input-bug-desc" />
            {!preDesc && <p className="text-xs text-[#A8ABB0] mt-1 mb-0">该用例暂无失败步骤，未预填。</p>}
          </div>
        ...
        <div>
          ...
          <Table
            ...
            pagination={false}
            columns={[
              { title: '编号', dataIndex: 'num', width: 90, render: (n: number) => <span className="text-[#87888D]">BUG-{String(n).padStart(4, '0')}</span> },
