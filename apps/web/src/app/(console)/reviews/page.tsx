...
export default function ReviewListPage() {
  ...
  const copy = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      message.success("已复制（名称 +copy，评审结果全部重置）");
    },
        ...
        <Table<ReviewRow>
          ...
          columns={[
              ...
              render: (v: string) => {
                ...
                return (
                  <Tooltip title={info.tip}>
                    <Tag color={v === "MULTI" ? "blue" : "default"} className="cursor-help">
                      {info.label}
                    </Tag>
                  </Tooltip>
                );
            ...
            },
            {
              ...
              dataIndex: "reviewers",
              width: 130,
              render: (ids: string[]) => <AvatarStack ids={ids} nameOf={nameOf} />,
            },
              ...
              render: (v: number, row) => (
                <div className="flex items-center gap-2" data-testid="review-pass-rate">
                  ...
                  <span className="text-xs text-[#87888D]">
                    {v}%（{row.stats.judged}/{row.caseCount} 已评）
                  </span>
                </div>
              ),
            },
            {
              ...
              render: (v: string, row) => (
                <span className="whitespace-nowrap">
                  <Tag color={v === "UNDERWAY" ? "processing" : "default"}>
                    {v === "UNDERWAY" ? "进行中" : "已结束"}
                  </Tag>
            ...
            },
            {
              ...
              render: (_, row) => (
                <span className="flex gap-2 whitespace-nowrap">
                  ...
                  {canUpdate && row.status === "UNDERWAY" && (
                    <Popconfirm
                      title={`删除评审「${row.name}」？仅解除关联，不影响用例本身。`}
      ...
      >
        <div className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[13px] mb-1">
                评审人 <span className="text-[#FF4D4F]">*</span>
              </label>
              <MemberSelect
                projectId={projectId}
                mode="multiple"
