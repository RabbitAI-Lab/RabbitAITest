...
export default function ReviewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const selected =
    cases.find((c) => c.caseId === selectedCaseId) ?? filtered[0] ?? cases[0] ?? null;
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      fail,
      suggest,
      judged: pass + fail + suggest,
      passRate: total === 0 ? 0 : Math.round((pass / total) * 100),
    };
  }, [cases]);
  ...
  const submitDisabled =
    !judgeable || !selected || !judgeResult || (commentRequired && !comment.trim());
  const lastJudgeText = (c: ReviewCaseRow) => {
    const last = c.results[c.results.length - 1];
    return last
      ? `${nameOf(last.userId)} · ${last.ts.slice(5, 10)} ${RESULT_META[last.result]?.label ?? last.result}`
      : "待评审";
  };
    ...
    <div>
      ...
      <div className="rabbit-card p-4 flex items-center gap-6 mb-4">
        <div className="flex-1 min-w-0">
          <p className="text-xs text-[#A8ABB0] mb-1">
            <a href="/reviews" className="text-[#87888D] hover:text-[#574BFF] no-underline">
              用例评审
            </a>{" "}
            / {review.name}
          </p>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Tooltip title={MODE_TIP[review.reviewMode]}>
              ...
              >
                {review.reviewMode === "MULTI" ? "多人评审" : "单人评审"} ⓘ
              </Tag>
            </Tooltip>
            <Tag color={ended ? "default" : "processing"}>{ended ? "已结束" : "进行中"}</Tag>
          </div>
          <p className="text-xs text-[#87888D] mt-2 mb-0">
            {review.startAt ? review.startAt.slice(0, 10) : "—"} ~{" "}
            {review.endAt ? review.endAt.slice(0, 10) : "—"}
            {review.endAt && !ended && new Date(review.endAt).getTime() < Date.now() && (
              <Tag color="warning" className="!ml-2">
                已逾期
              </Tag>
            )}
            <span className="ml-3">
              已评 {stats.judged} / {stats.total} · 通过 {stats.pass} · 失败 {stats.fail} · 建议{" "}
              {stats.suggest} · 未评审 {stats.total - stats.judged}
            </span>
          </p>
        </div>
        <Progress
        ...
        {can("PROJECT_CASE_REVIEW:UPDATE") && !ended && (
          ...
          >
            <Button danger data-testid="btn-close-review-detail">
              结束评审
            </Button>
          </Popconfirm>
        )}
      ...
      <div className="flex items-stretch gap-4">
        {/* 左：用例清单 */}
        <div
          className="w-80 shrink-0 rabbit-card !rounded-[10px] flex flex-col"
          ...
          <div className="flex-1 overflow-y-auto max-h-[calc(100vh-320px)] text-[13px]">
            {filtered.length === 0 && (
              <Empty
                className="my-10"
                description={
                  cases.length === 0 ? "暂未关联用例，点击右上「关联用例」" : "无该状态用例"
                }
              />
            )}
            {filtered.map((c) => {
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{ background: resultMeta(c.result).color }}
                  />
                  <span className="flex-1 min-w-0">
                    <span className="block truncate text-[#3D4350]">
                      {padNum(c.num)} {c.name}
                    </span>
        ...
        <div className="flex-1 min-w-0 rabbit-card !rounded-[10px] p-4 flex flex-col">
          {!selected ? (
            ...
            <>
              <div className="flex items-center gap-2 flex-wrap">
                <a className="text-[#574BFF] font-medium" href={`/cases/${selected.caseId}`}>
                  {padNum(selected.num)} {selected.name}
                </a>
                ...
                <Button
                  ...
                  data-testid="btn-review-history"
                >
                  评审历史
                </Button>
              </div>
              ...
              <div className="mt-3 flex-1">
                ...
                />
                {selected.reSubmit && (
                  <p className="mt-2 text-xs text-[#FA8C16] m-0">
                    该用例评审后被编辑过，已重置为未评审，待重新评审。
                  </p>
                )}
              </div>
            </>
      ...
      {review.isReviewer && !ended && (
        <div className="rabbit-card !rounded-[10px] p-3 mt-4 flex items-center gap-3 flex-wrap">
          ...
          {commentRequired && comment.trim() === "" && (
            <span className="text-xs text-[#FF4D4F]">选择失败 / 建议时必须填写评审意见</span>
          )}
        </div>
      )}
      {!review.isReviewer && !ended && (
          ...
          <div className="relative pl-4 max-h-96 overflow-y-auto" data-testid="review-history">
            ...
            {[...selected.results].reverse().map((r, i) => (
              <div key={i} className="relative py-2 border-b border-[#F0F1F3] last:border-0">
                <span
                  className="absolute -left-4 top-4 w-[7px] h-[7px] rounded-full"
                  style={{ background: RESULT_META[r.result]?.color ?? "#574BFF" }}
                />
                <p className="text-[13px] m-0">
                  <span className="font-medium mr-2">{nameOf(r.userId)}</span>
                  <Tag color={RESULT_META[r.result]?.tag ?? "default"} className="!mr-2">
                ...
                {r.comment && (
                  <p className="text-[13px] text-[#3D4350] whitespace-pre-wrap mt-1 mb-0">
                    {r.comment}
                  </p>
                )}
              </div>
...
function LinkCasesModal({
  ...
  const toTree = (
    nodes: { id: string; name: string; subtreeCount: number; children: unknown[] }[],
  ): DataNode[] =>
    nodes.map((n) => ({
      key: n.id,
      title: (
      ...
      children: (n.children as Parameters<typeof toTree>[0]).length
        ? toTree(n.children as Parameters<typeof toTree>[0])
        : undefined,
    }));
  const findNode = (
  ...
  const selectedNode = findNode(
    mods?.items as { id: string; children: unknown[] }[] | undefined,
    moduleId,
  );
  const isParent = Boolean(
    selectedNode && (selectedNode.children as unknown[] | undefined)?.length,
  );
