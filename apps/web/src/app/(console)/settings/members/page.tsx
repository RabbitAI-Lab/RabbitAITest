...
export default function SettingsMembersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: membersKey,
    queryFn: () =>
      projectInfoApi.members(currentProjectId!, {
  ...
  const invalidateMembers = () =>
    qc.invalidateQueries({ queryKey: ["project-members", currentProjectId] });
            ...
            columns={[
              {
                title: "邮箱",
                dataIndex: "email",
                render: (v: string) => <span data-testid="member-email">{v}</span>,
              },
                    ...
                    <span className="flex gap-1 flex-wrap">
                      {gs.map((g) => (
                        <Tag
                          key={g.name}
                          bordered={false}
                          color={g.isSystem ? "default" : "purple"}
                        >
                          {g.name}
                        </Tag>
                      ))}
          ...
          <div className="p-2">
            {(groups ?? []).map((g) => (
              ...
              >
                ...
                {g.isSystem ? (
                  <span className="ml-auto text-xs text-[#A8ABB0] shrink-0">
                    {g.memberCount} 人
                  </span>
                ) : (
                  <Tag className="ml-auto !mr-0" bordered={false}>
                    自定义组 · {g.memberCount} 人
                  </Tag>
                )}
              </Link>
            ))}
            {(groups ?? []).length === 0 && (
              <p className="px-3 py-2 text-xs text-[#C0C4CC]">暂无用户组</p>
            )}
            <p className="px-3 pt-2 pb-1 text-xs text-[#A8ABB0]">
              点击用户组进入组详情：成员维护 + 项目级权限点勾选
            </p>
            <div className="p-2">
              <Link
                href="/settings/groups"
                className="block text-center text-[13px] text-[#574BFF] hover:opacity-80"
                data-testid="link-settings-groups"
              >
                管理用户组 →
              </Link>
            </div>
