...
export default function TemplateSettingsPage() {
  ...
  useEffect(() => {
    if (scene === "case" && tab === "workflow") setTab("fields");
  }, [scene, tab]);
  ...
  const openFieldEdit = (row: FieldDefRow) => {
    ...
    setFieldForm({
      ...
      min: o.min as number | undefined,
      max: o.max as number | undefined,
      minLength: o.minLength as number | undefined,
      maxLength: o.maxLength as number | undefined,
      pattern: (o.pattern as string | undefined) ?? "",
  ...
  const buildOptions = (): Record<string, unknown> => {
    ...
    if (["single_select", "multi_select", "radio"].includes(fieldForm.type)) {
      const list = fieldForm.optionsText
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    if (["input", "textarea", "url"].includes(fieldForm.type)) {
      if (fieldForm.minLength !== undefined && fieldForm.minLength !== null)
        opts.minLength = fieldForm.minLength;
      if (fieldForm.maxLength !== undefined && fieldForm.maxLength !== null)
        opts.maxLength = fieldForm.maxLength;
  ...
  const saveField = useMutation({
    mutationFn: async () => {
      const body: Record<string, unknown> = {
        scene: editingField ? editingField.scene : scene,
      ...
      if (editingField) {
        const dv = (editingField.options as { defaultValue?: string | number | string[] | boolean })
          .defaultValue;
  ...
  useEffect(() => {
    setBindings(bindingTpl?.fields ?? []);
  }, [bindingTpl]);
  const sceneDefs = (fieldsQ.data ?? []).filter((f) => f.scene === scene);
  const bindableDefs = sceneDefs.filter(
    (d) => d.enabled && !bindings.some((b) => b.fieldKey === d.key),
  );
  ...
  const saveTransitions = useMutation({
    mutationFn: () =>
      workflowApi.updateTransitions(
        projectId!,
        [...checked].map((s) => {
          const [fromSerial = "", toSerial = ""] = s.split("→");
  ...
  const createState = useMutation({
    mutationFn: (body: { serial: string; isEnd?: boolean }) =>
      workflowApi.createState(projectId!, body),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["workflow"] });
    ...
    <div>
            ...
            columns={[
              {
                ...
                dataIndex: "name",
                render: (v: string, r) => (
                  <span className={r.enabled ? "" : "text-[#A8ABB0]"}>{v}</span>
                ),
              },
              {
                title: "标识 key",
                ...
                render: (v: string, r) => (
                  <span
                    className={`font-mono text-xs ${r.enabled ? "text-[#646A73]" : "text-[#A8ABB0]"}`}
                  >
                    {v}
                  </span>
                ),
              },
              {
                ...
                width: 96,
                render: (v: string, r) => (
                  <span className={r.enabled ? "" : "text-[#A8ABB0]"}>
                    {TYPE_LABELS[v as FieldType] ?? v}
                  </span>
                ),
              },
              {
              ...
              },
              {
                ...
                render: (v: boolean) =>
                  v ? (
                    <span className="text-xs text-[#52C41A]">启用</span>
                  ) : (
                    <span className="text-xs text-[#A8ABB0]">停用</span>
                  ),
              },
              {
                title: "操作",
                  ...
                  canUpdate ? (
                    <span className="flex gap-2">
                      <Button
                        ...
                        size="small"
                        className="!px-0"
                        onClick={() => openFieldEdit(row)}
                      >
                        编辑
                      </Button>
                      <Popconfirm
                        title={`删除字段「${row.name}」？`}
                        description="若已有存量数据将转为停用（数据保留）"
                        onConfirm={() => removeField.mutate(row.id)}
                      >
            ...
            <div className="rabbit-card p-3 flex items-center gap-3">
              ...
              {canUpdate && (
                <Button
                  ...
                  data-testid="btn-enable-project-template"
                >
                  启用项目模板
                </Button>
              )}
          ...
          <div className="rabbit-card">
            <div className="flex items-center gap-3 p-3 border-b border-[#F0F1F3]">
              ...
              <span className="text-xs text-[#A8ABB0]">
                {sceneLabel}模板上限 20 个；每个场景有且仅有一个默认模板
              </span>
              {canUpdate && (
                <Button
                  type="primary"
              ...
              columns={[
                {
                  ...
                  render: (v: string, r) => (
                    <span>
                      <a className="text-[#574BFF]" onClick={() => canUpdate && setBindingTpl(r)}>
                        {v}
                      </a>
                      {r.isDefault && (
                        <Tag color="purple" bordered={false} className="ml-2">
                          默认
                        </Tag>
                      )}
                      {r.isSystem && (
                        <Tag bordered={false} className="ml-1">
                          系统
                        </Tag>
                      )}
                    </span>
                ...
                },
                {
                  ...
                  dataIndex: "fields",
                  width: 80,
                  render: (f: TemplateRow["fields"]) => f.length,
                },
                { title: "引用数", dataIndex: "refCount", width: 80 },
                {
                  ...
                  dataIndex: "createdAt",
                  width: 120,
                  render: (v: string) => <span className="text-[#87888D]">{v.slice(0, 10)}</span>,
                },
                {
                  title: "操作",
                  key: "op",
                    ...
                    canUpdate ? (
                      <span className="flex gap-1">
                        ...
                        <Button
                          ...
                          size="small"
                          className="!px-0"
                          onClick={() => copyTemplate.mutate(r.id)}
                        >
                          复制
                        </Button>
                        <Button
                        ...
                        {!r.isSystem && (
                          <Popconfirm
                            title={`删除模板「${r.name}」？`}
                            description="存在实例引用时将被拒绝"
                            onConfirm={() => removeTemplate.mutate(r.id)}
                          >
                            <Button type="link" size="small" danger className="!px-0">
                              删除
                            </Button>
            ...
            <div className="flex items-center p-3 border-b border-[#F0F1F3]">
              ...
              {canUpdate && (
                <Button
                  ...
                  onClick={promptCreateState}
                  data-testid="btn-new-state"
                >
                  ＋ 新建状态
                </Button>
              )}
            ...
            <div className="p-2 space-y-1 flex-1">
              {wf.states.map((s, i) => (
                ...
                >
                  ...
                  {s.isStart && (
                    <span className="text-[10px] border border-[#574BFF] text-[#574BFF] rounded px-1">
                      初始
                    </span>
                  )}
                  {s.isEnd && (
                    <span className="text-[10px] border border-[#52C41A] text-[#52C41A] rounded px-1">
                      结束
                    </span>
                  )}
                  ...
                  {canUpdate && !s.isStart && (
                    <Popconfirm
                      title={`删除状态「${s.serial}」？`}
                      description="需先清除矩阵中相关勾选"
                      onConfirm={() => removeState.mutate(s.id)}
                    >
                      <Button type="link" size="small" danger className="!px-0 !h-auto">
                        删除
                      </Button>
                    </Popconfirm>
                  )}
            ...
            <div className="p-3 border-b border-[#F0F1F3] flex items-center gap-2">
              ...
              {canUpdate && (
                <Button
                  ...
                  size="small"
                  className="ml-auto"
                  loading={saveTransitions.isPending}
                  onClick={() => saveTransitions.mutate()}
                  data-testid="btn-save-transitions"
                >
                  保存矩阵
                </Button>
              )}
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="border-b border-[#F0F1F3] text-xs text-[#A8ABB0]">
                    ...
                    {wf.states.map((s) => (
                      <th key={s.id} className="p-2 font-normal whitespace-nowrap">
                        {s.serial}
                      </th>
                    ))}
                  </tr>
      ...
      {tab === "workflow" && scene === "bug" && !wf && !workflowQ.isLoading && (
        <Alert
          type="warning"
          showIcon
          message="默认缺陷模板缺失，无法加载工作流（请重新初始化组织模板）"
        />
      )}
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-[13px] mb-1">
              标识 key <span className="text-[#FF4D4F]">*</span>
            </label>
            <Input
              value={fieldForm.key}
            ...
            <p className="text-xs text-[#A8ABB0] mt-1">
              {editingField ? "key 只读：保存后不可修改" : "保存后不可修改；模板与实例按 key 引用"}
            </p>
          </div>
          <div>
            <label className="block text-[13px] mb-1">
              类型 <span className="text-[#FF4D4F]">*</span>
            </label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <InputNumber
                  className="w-full"
                  value={fieldForm.min}
                  onChange={(v) => setFieldForm({ ...fieldForm, min: v ?? undefined })}
                />
              </div>
              <div>
                ...
                <InputNumber
                  className="w-full"
                  value={fieldForm.max}
                  onChange={(v) => setFieldForm({ ...fieldForm, max: v ?? undefined })}
                />
              </div>
            </div>
          )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <InputNumber
                    ...
                    onChange={(v) => setFieldForm({ ...fieldForm, minLength: v ?? undefined })}
                  />
                </div>
                <div>
                  ...
                  <InputNumber
                    ...
                    onChange={(v) => setFieldForm({ ...fieldForm, maxLength: v ?? undefined })}
                  />
                </div>
              </div>
              <div>
          ...
          {fieldForm.type === "member" && (
            <div className="flex items-center gap-2">
              ...
              <Switch
                ...
                onChange={(v) => setFieldForm({ ...fieldForm, multiple: v })}
              />
            </div>
          )}
          <div className="flex items-center gap-2">
          ...
          {canUpdate && (
            <Button
              ...
              disabled={
                !fieldForm.name.trim() || !/^[a-z][a-z0-9_]{1,63}$/.test(fieldForm.key.trim())
              }
      ...
      <Modal
        ...
        okButtonProps={{
          danger: true,
          disabled: enableName.trim() !== (project?.name ?? ""),
          loading: enableMode.isPending,
        }}
      ...
      >
        ...
        <p className="text-[13px] mb-1">
          输入项目名称 <b>{project?.name}</b> 以确认：
        </p>
        <Input
          value={enableName}
          onChange={(e) => setEnableName(e.target.value)}
          placeholder="项目名称"
          data-testid="input-enable-template-name"
        />
      </Modal>
      ...
      >
          ...
          columns={[
              ...
              render: (_, b) => {
                ...
                return (
                  <span className="text-[#646A73]">
                    {def ? TYPE_LABELS[def.type as FieldType] : "—"}
                  </span>
                );
              },
            },
            {
              ...
              render: (_, b, i) => (
                <Checkbox
                  checked={
                    b.required ?? sceneDefs.find((d) => d.key === b.fieldKey)?.required ?? false
                  }
                  onChange={(e) =>
                    setBindings(
                      bindings.map((x, j) => (j === i ? { ...x, required: e.target.checked } : x)),
                    )
                  }
                />
              ),
              ...
              render: (_, b, i) => (
                <Checkbox
                  ...
                  onChange={(e) =>
                    setBindings(
                      bindings.map((x, j) =>
                        j === i ? { ...x, visibleInList: e.target.checked } : x,
                      ),
                    )
                  }
                />
        ...
        {bindableDefs.length > 0 && (
          <div className="flex items-center gap-2 mt-3">
            ...
            <Select
              ...
              onChange={(key) => {
                if (typeof key === "string") setBindings([...bindings, { fieldKey: key }]);
              }}
        ...
        <Button
          type="primary"
          className="mt-4"
          loading={saveBindings.isPending}
          onClick={() => saveBindings.mutate()}
          data-testid="btn-save-bindings"
        >
          保存绑定
        </Button>
      </Drawer>
