...
export default function OrgTemplatesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="组织模板管理"
        sub="组织级字段定义与模板（项目启用项目模板前，全组织项目共用）"
      />
      <div className="rabbit-card p-4">
        <Tabs
          items={[
            {
              key: "fields",
              label: <span data-testid="tab-fields">字段</span>,
              children: <FieldsTab orgId={orgId} editable={editable} />,
            },
            {
              key: "templates",
              label: <span data-testid="tab-templates">模板</span>,
              children: <TemplatesTab orgId={orgId} editable={editable} />,
            },
          ]}
        />
      </div>
...
function FieldsTab({ orgId, editable }: { orgId: string; editable: boolean }) {
  ...
  const [form, setForm] = useState<{
    ...
    required: boolean;
    options: string;
  }>({ name: "", key: "", type: "input", required: false, options: "" });
  ...
  const create = useMutation({
    mutationFn: () =>
      fieldDefApi.create(orgId, {
        ...
        options:
          form.type === "single_select" || form.type === "multi_select" || form.type === "radio"
            ? {
  ...
  const remove = useMutation({
    mutationFn: (id: string) =>
      fieldDefApi.remove(orgId, id) as unknown as Promise<{
    ...
    <div>
            ...
            ? [
                {
                  ...
                  render: (_: unknown, row: FieldDefRow) => (
                    <Popconfirm
                      title={
                        row.enabled ? "删除字段？有存量值时将转为停用。" : "彻底删除该停用字段？"
                      }
      ...
      <Drawer title="新建字段" open={drawer} onClose={() => setDrawer(false)} width={420}>
        <div className="space-y-4">
          <div>
            <label className="block text-[13px] mb-1">名称 *</label>
            <Input
              value={form.name}
          ...
          <div>
            <label className="block text-[13px] mb-1">标识（小写字母/数字/下划线）*</label>
            <Input
              value={form.key}
          ...
          <div className="flex items-center gap-2">
            <Switch checked={form.required} onChange={(v) => setForm({ ...form, required: v })} />
            必填
          </div>
          <Button
            type="primary"
            ...
            disabled={!form.name.trim() || !/^[a-z][a-z0-9_]{1,63}$/.test(form.key)}
...
function TemplatesTab({ orgId, editable }: { orgId: string; editable: boolean }) {
  ...
  return (
    <div>
        ...
        columns={[
          {
            ...
            render: (v: string, row) => (
              <span>
                ...
                {row.isSystem && <Tag className="ml-1">系统</Tag>}
              </span>
            ),
          },
      ...
      />
      <p className="text-xs text-[#A8ABB0] mt-3">
        字段绑定与覆写、缺陷工作流在「项目设置 › 模板管理」按项目上下文编辑。
      </p>
    </div>
  );
}
