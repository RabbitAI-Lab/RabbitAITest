...
export default function OrgProjectsPage() {
  ...
  const [editForm, setEditForm] = useState<{ name: string; description: string; modules: Record<string, boolean> }>({
    name: '', description: '', modules: { case: true, plan: true, bug: true, api: true },
  });

  const { data, isLoading } = useQuery({
    queryKey: ['org-projects', orgId, tab, keyword],
    queryFn: () => orgApi.projects(orgId!, { deleted: tab === 'deleted' ? '1' : undefined, keyword: keyword || undefined }),
  ...
  const create = useMutation({
    mutationFn: () => orgApi.createProject(orgId!, { name: createForm.name.trim(), description: createForm.description.trim() || undefined }),
    onSuccess: () => {
      invalidate();
      setCreateOpen(false);
  ...
  const openEdit = (row: OrgProjectRow) => {
    setEditTarget(row);
    setEditForm({ name: row.name, description: row.description ?? '', modules: { case: true, plan: true, bug: true, api: true, ...row.modules } });
  };
    ...
    <div>
        ...
        extra={
          <div className="flex items-center gap-2">
            ...
            {can('ORG_PROJECT:CREATE') && (
              <Button type="primary" icon={<Plus size={14} />} onClick={() => setCreateOpen(true)} data-testid="btn-new-project">
                新建项目
              </Button>
            )}
          </div>
        ...
        {tab === 'list' ? (
            ...
            columns={[
              {
                ...
                render: (v: string, row) => (
                  <div>
                    <a className="text-[#1F2329] hover:text-[#574BFF] font-medium" onClick={() => enter(row)} data-testid={`project-name-${row.num}`}>{v}</a>
                    {row.description && <p className="text-xs text-[#A8ABB0] mt-0.5">{row.description}</p>}
                  </div>
                ),
              },
              { title: '编号', dataIndex: 'num', width: 80, render: (n: number) => <span className="text-[#87888D]">#{n}</span> },
              { title: '成员数', dataIndex: 'memberCount', width: 90, render: (n: number) => `${n} 人` },
              {
                title: '状态', dataIndex: 'status', width: 120,
                render: (v: string) => (v === 'ENDED' ? <Tag>已结束（只读）</Tag> : <Tag color="success">启用</Tag>),
              },
              { title: '创建时间', dataIndex: 'createdAt', width: 110, render: (v: string) => <span className="text-[#87888D]">{fmt(v)}</span> },
              {
                title: '操作', key: 'op', width: 260,
                render: (_, row) => (
                  <span className="flex items-center gap-1 text-[13px]">
                    <Button type="link" size="small" className="!px-0" onClick={() => enter(row)}>进入</Button>
                    <span className="text-[#E5E6EB]">|</span>
                    {can('ORG_PROJECT:UPDATE') && (
                      <>
                        <Button type="link" size="small" className="!px-0" onClick={() => openEdit(row)}>编辑</Button>
                        <span className="text-[#E5E6EB]">|</span>
                        {row.status === 'ENDED' ? (
                          <Button type="link" size="small" className="!px-0" onClick={() => reopenProject.mutate(row.id)}>开启</Button>
                        ) : (
                          <Popconfirm
                            title="结束项目"
                          ...
                          >
                            <Button type="link" size="small" className="!px-0">结束</Button>
                          </Popconfirm>
                        )}
                        <span className="text-[#E5E6EB]">|</span>
                      </>
                    ...
                    {can('ORG_PROJECT:DELETE') && (
                      <Button type="link" size="small" danger className="!px-0" onClick={() => confirmDelete(row)} data-testid="btn-delete-project">
                        删除
                      </Button>
                    )}
                  </span>
          ...
          <Table<OrgProjectRow>
            ...
            columns={[
              ...
              { title: '删除时间', dataIndex: 'deletedAt', width: 110, render: (v: string | null) => <span className="text-[#87888D]">{fmt(v)}</span> },
              {
                title: '剩余可恢复时间', key: 'purge', width: 200,
                render: (_, row) => {
                  const days = row.purgeAt
                    ? Math.max(0, Math.ceil((new Date(row.purgeAt).getTime() - Date.now()) / 86_400_000))
                    : 0;
                  return <span className="text-[#87888D]">{days > 0 ? `剩余 ${days} 天自动清除` : '即将清除'}</span>;
                },
              },
              {
      ...
      >
        <div className="space-y-4 pt-2">
          ...
          <p className="text-xs text-[#A8ABB0]">创建后可在「设置 › 基本信息」中配置模块开关；模块默认全部开启。</p>
        </div>
      </Modal>
              ...
              {MODULE_OPTIONS.map((m) => (
                <div key={m.key} className="flex items-center gap-3">
                  <Switch
                    ...
                    onChange={(v) => setEditForm({ ...editForm, modules: { ...editForm.modules, [m.key]: v } })}
