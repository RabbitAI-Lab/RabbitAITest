...
export default function NewBugPage() {
  ...
  const defaultBindings: TemplateFieldBinding[] | undefined = defaultTpl?.fields?.map((b) => ({
    ...b,
      ...
      <div className="rabbit-page-header">
        <div className="flex-1 min-w-0">
          ...
          <h1>
            新建缺陷
            {defaultTpl && (
              <span className="ml-2 text-xs font-normal text-[#A8ABB0]">
                模板：{defaultTpl.name}（默认）
              </span>
            )}
          </h1>
        </div>
      ...
      <div className="rabbit-card p-6 space-y-5" data-testid="bug-form">
        ...
        <div className="grid grid-cols-[80px_1fr] gap-3">
          ...
          <div>
            <Input.TextArea
              ...
              maxLength={8000}
              value={description}
              placeholder="支持受限 Markdown（标题/列表/代码块/表格）"
        ...
        <div className="grid grid-cols-[80px_1fr] items-center gap-3">
          ...
          <MemberSelect
            projectId={projectId!}
            value={handleUserId}
            onChange={(v) => setHandleUserId(Array.isArray(v) ? v[0] : v)}
            placeholder="选择处理人"
          />
        </div>
        <div className="grid grid-cols-[80px_1fr] items-center gap-3">
        ...
        <div>
          ...
          />
          {defs.length === 0 && (
            <p className="text-[#A8ABB0] text-[13px]">当前无启用的缺陷自定义字段</p>
          )}
        </div>
        <div
          className="border border-dashed border-[#E5E6EB] rounded-md p-4 flex items-center gap-2 text-[13px] text-[#A8ABB0]"
