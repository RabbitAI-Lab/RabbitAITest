...
function statusColor(
  serial: string,
  states?: { serial: string; isStart: boolean; isEnd: boolean }[],
): string {
...
export default function BugDetailPage() {
  ...
  const caseSearchQ = useQuery({
    ...
    queryFn: () =>
      caseApiV2.list(projectId!, { page: 1, pageSize: 20, keyword: linkKeyword || undefined }),
  ...
  const tplBindings: TemplateFieldBinding[] | undefined = (() => {
    ...
    const tpl =
      (bug.templateId ? list.find((t) => t.id === bug.templateId) : null) ??
  ...
  const visibleDefs = tplBindings
    ? defs.filter((d) => tplBindings.some((b) => b.fieldKey === d.key))
    : defs;
  ...
  const follow = useMutation({
    ...
    onSuccess: () => {
      setFollowed((v) => !v);
      message.success(followed ? "已取消关注" : "已关注，变更将提醒");
    },
    onError: (e) => message.error(e instanceof Error ? e.message : "操作失败"),
  });
  const saveEdit = useMutation({
    mutationFn: () =>
      bugApi.update(projectId!, id, {
        title: form!.title.trim(),
        description: form!.description,
        handleUserId: form!.handleUserId ?? null,
        tags: form!.tags,
        fields: form!.fields,
        version: bug!.version,
      }),
  ...
  const tabs = [
    ...
    {
      key: "comments",
      label: `评论（${(commentsQ.data?.items ?? []).filter((c) => !c.content.startsWith(FLOW_PREFIX)).length ?? 0}）`,
      testid: "tab-comments",
    },
    ...
    if (bugQ.isError) {
      return (
        <div className="rabbit-card p-6">
          <p className="text-[13px] text-[#FF4D4F] mb-3" data-testid="bug-load-error">
            {bugQ.error instanceof Error ? bugQ.error.message : "缺陷加载失败"}
          </p>
          <a className="text-[#574BFF] text-[13px]" href="/bugs">
            ‹ 返回缺陷列表
          </a>
        </div>
      );
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <a className="text-[13px] text-[#87888D] hover:text-[#574BFF] no-underline" href="/bugs">
          ‹ 缺陷列表
        </a>
        ...
        <span
          ...
          style={{
            background: `${statusColor(bug.status, states)}1A`,
            color: statusColor(bug.status, states),
          }}
        >
          {bug.status}
        </span>
        <div className="ml-auto flex gap-2 items-center">
          {canUpdate &&
            bug.allowedTransitions.map((to) => (
              <Button
                key={to}
                data-testid={`btn-transition-${to}`}
                onClick={() => {
                  setTransitionTo(to);
                  setTransitionComment("");
                }}
          ...
          <Button
            icon={<Star size={14} className={followed ? "fill-[#FA8C16] text-[#FA8C16]" : ""} />}
            onClick={() => follow.mutate()}
            data-testid="btn-follow-bug"
          >
            {followed ? "已关注" : "关注"}
          </Button>
          <Button icon={<Share2 size={14} />} onClick={share} data-testid="btn-share-bug">
            分享
          </Button>
          {canUpdate && (
            <Button
              ...
              onClick={() => {
                ...
                setForm({
                  ...
                  handleUserId: bug.handleUserId ?? undefined,
                  tags: bug.tags,
                  fields: bug.fields,
                });
                setEditing(true);
      ...
      <p className="text-xs text-[#A8ABB0] mt-1.5 mb-4">
        创建人 {memberName(bug.createdBy) ?? bug.createdBy.slice(0, 8)} ·{" "}
        {bug.createdAt.replace("T", " ").slice(0, 16)} · 更新{" "}
        {bug.updatedAt.replace("T", " ").slice(0, 16)} · v{bug.version}
      </p>
      ...
      {editing && tab === "detail" && (
        <div className="rabbit-card p-6 space-y-5 mb-4" data-testid="bug-edit-form">
          <div className="grid grid-cols-[80px_1fr] items-center gap-3">
            <label className="text-[13px] text-[#3D4350]">
          ...
          <div className="grid grid-cols-[80px_1fr] items-center gap-3">
            ...
            <MemberSelect
              projectId={projectId!}
              value={form!.handleUserId}
              onChange={(v) => setForm({ ...form!, handleUserId: Array.isArray(v) ? v[0] : v })}
            />
          </div>
          <div className="grid grid-cols-[80px_1fr] items-center gap-3">
      ...
      {tab === "detail" && !editing && (
        <div className="rabbit-card p-5 max-w-4xl space-y-5" data-testid="bug-detail-panel">
          <div>
            ...
            <MarkdownView
              md={bug.description}
              className="bg-[#F7F8FA] rounded p-3 text-[#646A73]"
            />
          </div>
          {visibleDefs.length > 0 && (
            <div>
              ...
              <div
                className="grid grid-cols-3 gap-x-6 gap-y-3 text-[13px]"
                data-testid="bug-dynamic-fields"
              >
                {visibleDefs.map((d) => (
                  <p key={d.key} className="text-[#646A73] flex gap-2">
                    <span className="shrink-0">{d.name}</span>
                    <span className="font-medium text-[#1F2329] min-w-0">
                      <DynamicFieldCell def={d} value={bug.fields[d.key]} />
                    </span>
                  </p>
                ))}
              </div>
                ...
                {(attachQ.data ?? []).map((a) => (
                  <div key={a.id} className="flex items-center gap-3 py-2 text-[13px]">
                    <span
                      ...
                      data-testid={`attachment-name-${a.id}`}
                    >
                      {a.name}
                    </span>
                    ...
                    <a
                      ...
                      href={bugApi.downloadUrl(projectId!, a.id)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      下载
                    ...
                    {canUpdate && (
                      <Popconfirm
                        title={`删除附件「${a.name}」？`}
                        onConfirm={() => removeAttachment.mutate(a.id)}
                      >
            ...
            columns={[
              {
                ...
                width: 100,
                render: (n: number) => (
                  <span className="text-[#87888D]">C-{String(n).padStart(4, "0")}</span>
                ),
              },
              {
                title: "名称",
                dataIndex: "name",
                render: (v: string, r) => (
                  <a className="text-[#574BFF]" href={`/cases/${r.caseId}`}>
                    {v}
                  </a>
                ),
              },
              {
                ...
                dataIndex: "level",
                width: 80,
                render: (l: string) => <Tag bordered={false}>{l}</Tag>,
              },
              {
                title: "操作",
                key: "op",
      ...
      {tab === "comments" && (
        <div className="rabbit-card p-5">
          <CommentThread
            projectId={projectId!}
            entity={`bug:${id}`}
            canModeratePerm="PROJECT_BUG:UPDATE"
      ...
      {tab === "history" && (
        <div className="rabbit-card p-5 max-w-4xl">
          <p className="text-xs text-[#A8ABB0] mb-2">
            口径：流转与编辑历史；当前版本以流转记录为准（取流转时自动写入的评论，按时间倒序）。
          </p>
      ...
      >
        ...
        <Select
          ...
          options={(caseSearchQ.data?.items ?? []).map((c) => ({
            value: c.id,
