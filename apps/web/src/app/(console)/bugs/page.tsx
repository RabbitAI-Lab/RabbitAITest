...
function statusColor(serial: string, wf?: { states: { serial: string; isStart: boolean; isEnd: boolean }[] }): string {
...
export default function BugListPage() {
  ...
  const listQ = useQuery({
    queryKey: ['bug', 'list', projectId, recycled, keyword, status, handler, moduleId, includeChildren, page],
    queryFn: () => bugApi.list(projectId!, {
      page, pageSize: 20,
      keyword: keyword || undefined, status, handler,
      moduleId: moduleId ?? undefined, includeChildren: moduleId ? includeChildren : undefined,
      recycled,
    }),
    enabled: Boolean(projectId),
  });
          ...
          <div className="flex items-center gap-2">
            <div className="flex bg-white border border-[#E5E6EB] rounded-md p-0.5 text-[13px]">
              ...
              <span
                ...
                className={`px-3 py-1 rounded cursor-pointer transition-colors ${recycled ? 'bg-[#574BFF]/8 text-[#574BFF] font-medium' : 'text-[#646A73]'}`}
                onClick={() => { setRecycled(true); setPage(1); setStatus(undefined); setHandler(undefined); }}
              >
                回收站
              </span>
            </div>
            {!recycled && can('PROJECT_BUG:CREATE') && (
              <Button type="primary" icon={<Plus size={14} />} onClick={() => router.push('/bugs/new')} data-testid="btn-new-bug">新建缺陷</Button>
            )}
          </div>
        }
      />
      <div className="flex gap-4 items-start">
        ...
        <div className="rabbit-card flex-1 min-w-0">
          <div className="flex gap-2 p-3 border-b border-[#F0F1F3] flex-wrap">
            ...
            />
            <Select
              className="w-36" allowClear placeholder="状态：全部"
              value={status} options={stateOptions}
            ...
            {!recycled && (
              <MemberSelect
                projectId={projectId!}
                value={handler}
                onChange={(v) => { setHandler(Array.isArray(v) ? v[0] : v); setPage(1); }}
                placeholder="处理人：全部"
              />
            )}
          </div>
          <Table<BugRow>
            ...
            columns={[
              { title: '编号', dataIndex: 'num', width: 96, render: (n: number) => <span className="text-[#87888D]">B-{String(n).padStart(4, '0')}</span> },
              {
                title: '标题', dataIndex: 'title',
                render: (v: string, r) => <a className="text-[#1F2329] hover:text-[#574BFF] font-medium" href={`/bugs/${r.id}`}>{v}</a>,
              },
              {
                title: '状态', dataIndex: 'status', width: 110,
                render: (v: string) => (
                  <span className="whitespace-nowrap">
                    <span className="inline-block w-2 h-2 rounded-full mr-1.5 align-middle" style={{ background: statusColor(v, wf) }} />
                    {v}
                  </span>
                ),
              },
              {
                title: '处理人', dataIndex: 'handleUserId', width: 90,
                render: (id: string | null) => {
                  if (!id) return <span className="text-[#A8ABB0]">—</span>;
                  const name = memberName(id);
                  return <span title={name} className="inline-flex items-center gap-1"><span className="w-6 h-6 rounded-full bg-[#574BFF]/10 text-[#574BFF] text-xs grid place-items-center">{name.slice(0, 1)}</span>{name}</span>;
                },
              },
              {
                ...
                render: (_, r) => {
                  ...
                  return sev ? <span className="text-xs font-medium text-[#FA8C16]">{String(sev)}</span> : <span className="text-[#A8ABB0]">—</span>;
                },
              },
              {
                title: '标签', dataIndex: 'tags', width: 160,
                render: (tags: string[]) => tags.length ? tags.map((t) => <Tag key={t} bordered={false} color="processing">{t}</Tag>) : <span className="text-[#C0C4CC]">—</span>,
              },
              { title: '更新时间', dataIndex: 'updatedAt', width: 120, render: (s: string) => <span className="text-[#87888D] text-xs">{s.replace('T', ' ').slice(5, 16)}</span> },
              {
                title: '操作', key: 'op', width: recycled ? 190 : 210,
                render: (_, r) => recycled ? (
                  <span className="flex items-center gap-1">
                    <Button type="link" size="small" icon={<RotateCcw size={13} />} onClick={() => restore.mutate(r.id)}>恢复</Button>
                    <Popconfirm
                      title="彻底删除"
                  ...
                  <span className="flex items-center gap-1">
                    <a className="text-[#574BFF] hover:opacity-80" href={`/bugs/${r.id}`}>编辑</a>
                    <span className="text-[#E5E6EB]">|</span>
                    <Button type="link" size="small" className="!px-0" icon={<Star size={13} />} onClick={() => follow.mutate(r.id)} data-testid={`btn-follow-${r.num}`}>关注</Button>
                    {can('PROJECT_BUG:DELETE') && (
                      <Popconfirm title={`删除缺陷「${r.title}」？`} description="删除后进入回收站，可恢复" onConfirm={() => remove.mutate(r.id)}>
