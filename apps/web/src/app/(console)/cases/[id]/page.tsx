...
function CaseDetailInner() {
  ...
  const depSearchQ = useQuery({
    ...
    queryFn: () =>
      caseApiV2.list(projectId!, { page: 1, pageSize: 20, keyword: depKeyword || undefined }),
  ...
  const memberName = (uid: string | null) =>
    membersQ.data?.items.find((m) => m.id === uid)?.name ?? "—";
  ...
  const bindings: TemplateFieldBinding[] | undefined = (() => {
    ...
    const tpl =
      (kase?.templateId ? list.find((t) => t.id === kase.templateId) : null) ??
      list.find((t) => t.isDefault);
    return tpl?.fields?.map((b) => ({ ...b, visibleInList: b.visibleInList ?? false }));
  })();
  const boundDefs = bindings
    ? defs.filter((d) => bindings.some((b) => b.fieldKey === d.key))
    : defs;
  ...
  useEffect(() => {
    if (!projectId) return;
    prefApi
      .get("case_tab", projectId)
      .then((r) => {
        if (typeof r.value === "string" && (TAB_KEYS as readonly string[]).includes(r.value))
          setTab(r.value as TabKey);
      })
      .catch(() => undefined);
  }, [projectId]);
    ...
    <div className="rabbit-card flex-1 min-w-0">
      ...
      <div className="p-2">
        ...
        {list.map((d) => (
          <div
            key={d.id}
            className="flex items-center gap-2 px-2 py-2 border-b border-[#F7F8FA] last:border-0"
          >
            <span className="text-xs text-[#87888D] shrink-0">
              C-{String(d.case.num).padStart(4, "0")}
            </span>
            <a className="text-[13px] text-[#574BFF] truncate flex-1" href={`/cases/${d.case.id}`}>
              {d.case.name}
            </a>
            <Tag bordered={false} color={levelColor[d.case.level]}>
              {d.case.level}
            </Tag>
            {canUpdate && (
  ...
  return (
    <div className="max-w-5xl">
      ...
      <p className="text-xs text-[#A8ABB0] mb-2" data-testid="case-breadcrumb">
        <a className="hover:text-[#574BFF]" href="/cases">
          测试用例
        </a>
        {modulePath && <> / {modulePath}</>}
      </p>
      <div className="flex items-center gap-3 flex-wrap mb-4">
        <h1 className="text-lg font-medium m-0" data-testid="case-title">
          {kase.name}
        </h1>
        <Tag color={STATUS_COLOR[kase.status] ?? "default"}>
          {STATUS_TEXT[kase.status] ?? kase.status}
        </Tag>
        <Tag bordered={false} color={levelColor[kase.level]}>
          {kase.level}
        </Tag>
        <span className="text-xs text-[#A8ABB0]">
          C-{String(kase.num).padStart(4, "0")} · v{kase.version}
        </span>
        <div className="ml-auto flex gap-2 items-center">
          ...
          <Button
            icon={<Share2 size={14} />}
            onClick={() => void share()}
            data-testid="btn-share-case"
          >
            分享
          </Button>
          <Button
            icon={<Copy size={14} />}
            onClick={() => copyCase.mutate()}
            data-testid="btn-copy-case"
            loading={copyCase.isPending}
          >
            复制
          </Button>
          {canUpdate && !editing && (
          ...
          {editing && (
            <Button onClick={() => setEditing(false)} data-testid="btn-cancel-edit">
              退出编辑
            </Button>
          )}
        </div>
      ...
      >
        {tabs.map((t) => (
          ...
          >
            {t.label}
            {t.badge !== undefined && (
              <span className="ml-1 text-xs bg-[#F2F3F5] rounded px-1">{t.badge}</span>
            )}
          </span>
        ))}
      </div>
          ...
          <div className="space-y-4">
            <div className="rabbit-card p-5">
              ...
              <div className="grid grid-cols-3 gap-y-3 gap-x-8 text-sm">
                <div>
                  <p className="text-xs text-[#A8ABB0] mb-0.5">所属模块</p>
                  <p>{modulePath || "—"}</p>
                </div>
                <div>
                  <p className="text-xs text-[#A8ABB0] mb-0.5">等级</p>
                  <p>
                    <span className={kase.level === "P0" ? "text-[#FF4D4F] font-medium" : ""}>
                      {kase.level}
                    </span>
                  </p>
                </div>
                <div>
                  <p className="text-xs text-[#A8ABB0] mb-0.5">状态</p>
                  <p>{STATUS_TEXT[kase.status] ?? kase.status}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs text-[#A8ABB0] mb-0.5">创建人 / 时间</p>
                  <p>
                    {memberName(kase.createdBy)} · {kase.createdAt.replace("T", " ").slice(0, 16)}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-[#A8ABB0] mb-0.5">最后更新</p>
                  <p>{kase.updatedAt.replace("T", " ").slice(0, 16)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rabbit-card p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs border border-[#E5E6EB] rounded px-2 py-0.5 text-[#87888D]">
                  支持受限 Markdown（粗体/列表/链接/代码块/表格）
                </span>
              </div>
              ...
              <table className="w-full text-sm" data-testid="case-steps-view">
                ...
                <tbody>
                  {kase.steps.length === 0 && (
                    <tr>
                      <td colSpan={3} className="py-3 text-[#A8ABB0] text-center">
                        暂无步骤
                      </td>
                    </tr>
                  )}
                  {kase.steps.map((s, i) => (
                    <tr key={i} className="border-b border-[#F7F8FA] last:border-0">
                      ...
                      <td className="py-2 align-top">
                        <MarkdownView md={s.desc} />
                      </td>
                      <td className="py-2 align-top">
                        <MarkdownView md={s.expect} />
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <div className="grid grid-cols-4 gap-y-3 gap-x-6 text-sm">
                  {boundDefs.map((d) => (
                    <div key={d.key}>
                      ...
                      <p>
                        <DynamicFieldCell def={d} value={kase.fields?.[d.key]} />
                      </p>
                    </div>
                  ))}
                </div>
      ...
      {tab === "dependencies" && (
        <div>
          {canUpdate && (
            <div className="mb-3 flex justify-end">
              <Button
                ...
                data-testid="btn-add-dependency"
              >
                ＋ 添加前置依赖
              </Button>
            </div>
          ...
          <p className="text-xs text-[#A8ABB0] mt-2">
            依赖关系双向同步：B 依赖 A 时，A 的后置列表自动出现 B；删除任一侧即解除。
          </p>
        </div>
      )}
          ...
          {reviews.length === 0 ? (
            <Empty
              ...
              description={
                <span>
                  暂未参与用例评审，可从
                  <a className="text-[#574BFF]" href="/reviews">
                    用例评审
                  </a>
                  页发起并将本用例加入范围
                </span>
            ...
            <table className="w-full text-sm" data-testid="case-review-table">
              <thead className="text-[#87888D] text-xs bg-[#F7F8FA]">
                <tr>
                  <th className="text-left p-3">评审名称</th>
                  <th className="text-left p-3 w-24">我的评审结果</th>
                  <th className="text-left p-3 w-20">重新提审</th>
                  <th className="text-left p-3 w-24">评审状态</th>
                  <th className="text-left p-3 w-40">时间</th>
                </tr>
              </thead>
              <tbody>
                {reviews.map((r) => (
                  <tr key={r.reviewId} className="border-t border-[#F0F1F3]">
                    <td className="p-3">
                      <a className="text-[#574BFF]" href={`/reviews/${r.reviewId}`}>
                        {r.name}
                      </a>
                    </td>
                    <td className="p-3">
                      {r.result ? (
                        <Tag bordered={false} color={REVIEW_RESULT[r.result]?.color ?? "default"}>
                          {REVIEW_RESULT[r.result]?.label ?? r.result}
                        </Tag>
                      ) : (
                        <span className="text-[#A8ABB0] text-xs">待评审</span>
                      )}
                    </td>
                    <td className="p-3">
                      {r.reSubmit && (
                    ...
                    <td className="p-3 text-xs text-[#87888D]">
                      {(r.endAt ?? r.startAt ?? "").replace("T", " ").slice(0, 16) || "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {plans.length === 0 ? (
            <Empty
              ...
              description={
                <span>
                  暂未关联测试计划，可从
                  <a className="text-[#574BFF]" href="/plans">
                    测试计划
                  </a>
                  页关联本用例
                </span>
            ...
            <table className="w-full text-sm" data-testid="case-plan-table">
              <thead className="text-[#87888D] text-xs bg-[#F7F8FA]">
                <tr>
                  <th className="text-left p-3">计划名称</th>
                  <th className="text-left p-3 w-24">计划状态</th>
                  <th className="text-left p-3 w-28">我的执行结果</th>
                </tr>
              </thead>
              <tbody>
                {plans.map((p) => (
                  <tr key={p.planId} className="border-t border-[#F0F1F3]">
                    <td className="p-3">
                      <a className="text-[#574BFF]" href={`/plans/${p.planId}`}>
                        {p.name}
                      </a>
                    </td>
                    <td className="p-3 text-xs">
                      {PLAN_STATUS[p.planStatus] ?? p.planStatus}
                      {p.archived && (
                    ...
                    <td className="p-3">
                      {p.myExecStatus ? (
                        <Tag
                          bordered={false}
                          color={EXEC_RESULT[p.myExecStatus]?.color ?? "default"}
                        >
                          {EXEC_RESULT[p.myExecStatus]?.label ?? p.myExecStatus}
                        </Tag>
                      ) : (
            ...
            <div className="ml-auto flex gap-2">
              {canUpdate && (
                <Button
                  ...
                  data-testid="btn-link-bug"
                >
                  关联已有缺陷
                </Button>
              )}
              <Button size="small" href="/bugs/new" data-testid="btn-new-bug-link">
                新建缺陷
              </Button>
            </div>
          </div>
              ...
              <tbody>
                {bugs.map((b) => (
                  <tr key={b.bugId} className="border-t border-[#F0F1F3]">
                    ...
                    <td className="p-3">
                      <a className="text-[#574BFF]" href={`/bugs/${b.bugId}`}>
                        {b.title}
                      </a>
                    </td>
                    ...
                    <td className="p-3">
                      {canUpdate ? (
                        <Popconfirm
                          title="解绑该缺陷？"
                          onConfirm={() => unlinkBug.mutate(b.bugId)}
                        >
      ...
      {tab === "comments" && (
        <div className="rabbit-card p-5">
          <CommentThread
            projectId={projectId}
            entity={`case:${id}`}
            canModeratePerm="PROJECT_CASE:UPDATE"
            currentUserId={meQ.data?.userId ?? ""}
          />
        </div>
      )}
      ...
      >
        ...
        <Select
          ...
          options={depCandidates.map((c) => ({
            value: c.id,
            label: `C-${String(c.num).padStart(4, "0")} ${c.name}`,
          }))}
          loading={depSearchQ.isLoading}
          data-testid="select-dep-target"
        />
        <p className="text-xs text-[#A8ABB0] mt-2">
          已排除本用例；建立后对方用例的后置列表将同步出现本用例。
        </p>
      </Modal>
      ...
      >
        ...
        <Select
          ...
          options={(bugSearchQ.data?.items ?? []).map((b) => ({
            value: b.id,
            label: `B-${String(b.num).padStart(4, "0")} ${b.title}`,
          }))}
