...
export default function SystemUsersPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: (u) => {
      ...
      Modal.success({
        title: '用户已创建',
        content: <p>初始密码（仅本次展示）：<b data-testid="initial-password">{u.initialPassword}</b></p>,
      });
    },
  ...
  const reset = useMutation({
    ...
    onSuccess: (r) => {
      Modal.success({ title: '密码已重置', content: <p>新密码（仅本次展示）：<b>{r.newPassword}</b></p> });
    },
    onError: (e) => message.error(e instanceof Error ? e.message : '重置失败'),
  });
  const toggle = useMutation({
    mutationFn: ({ id, status }: { id: string; status: 'ACTIVE' | 'DISABLED' }) => userApi.setStatus(id, status),
    onSuccess: () => { invalidate(); message.success('状态已更新（该用户全部会话已失效）'); },
    onError: (e) => message.error(e instanceof Error ? e.message : '操作失败'),
  });
  const remove = useMutation({
    mutationFn: (id: string) => userApi.remove(id),
    onSuccess: () => { invalidate(); message.success('用户已删除（软删，邮箱保留占用）'); },
  ...
  return (
    <div>
      <PageHeader
        ...
        extra={<Button type="primary" icon={<Plus size={14} />} onClick={() => setDrawerOpen(true)} data-testid="btn-new-user">新建用户</Button>}
      />
      <div className="rabbit-card">
        ...
        <Table<UserRow>
          ...
          columns={[
            { title: '邮箱', dataIndex: 'email', render: (v: string) => <span data-testid="user-email">{v}</span> },
            ...
            { title: '手机', dataIndex: 'phone', render: (v: string | null) => v ?? '—' },
            {
              ...
              render: (v: string, row) => (
                <Switch
                  ...
                  onChange={(checked) => toggle.mutate({ id: row.id, status: checked ? 'ACTIVE' : 'DISABLED' })}
                  data-testid={`user-status-${row.email}`}
                />
              ),
            },
            { title: '创建时间', dataIndex: 'createdAt', render: (v: string) => v.replace('T', ' ').slice(0, 16) },
            {
              title: '操作', key: 'op', width: 220,
              render: (_, row) => (
                <span className="flex gap-2">
                  <Button type="link" size="small" className="!px-0" onClick={() => reset.mutate(row.id)} data-testid={`btn-reset-${row.email}`}>重置密码</Button>
                  {row.email !== 'admin@rabbit.test' && (
                    <Popconfirm title={`删除用户「${row.name}」？登录将失效，业务数据保留。`} onConfirm={() => remove.mutate(row.id)}>
      ...
      <Drawer title="新建用户" open={drawerOpen} onClose={() => setDrawerOpen(false)} width={420}>
        <div className="space-y-4">
          <div>
            <label className="block text-[13px] mb-1">邮箱 <span className="text-[#FF4D4F]">*</span></label>
            <Input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="user@example.com" data-testid="input-user-email" />
          </div>
          <div>
            <label className="block text-[13px] mb-1">姓名 <span className="text-[#FF4D4F]">*</span></label>
          ...
          <div>
            ...
            <Input.Password value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="留空自动生成" />
          </div>
          <Button type="primary" block loading={create.isPending} onClick={() => create.mutate()} disabled={!form.email.trim() || !form.name.trim()} data-testid="btn-submit-user">创建</Button>
        </div>
      </Drawer>
    </div>
  );
