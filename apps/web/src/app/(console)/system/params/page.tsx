...
const GROUP_LABEL: Record<string, string> = { basic: '基础', smtp: '邮箱', file: '文件', cleanup: '数据清理' };
...
export default function SystemParamsPage() {
  ...
  const [smtp, setSmtp] = useState<SystemParams['smtp']>({ host: '', port: 465, user: '', pass: '', ssl: true, from: '' });
  ...
  const save = useMutation({
    mutationFn: ({ group, value }: { group: 'basic' | 'smtp' | 'file' | 'cleanup'; value: Record<string, unknown> }) =>
  ...
  const items = [
    {
      ...
      children: (
        <div className="rabbit-card p-5 space-y-4 max-w-3xl">
          <div>
            <span className={labelCls}>站点 URL <span className="text-[#FF4D4F]">*</span></span>
            <Input
            ...
            />
            {basic.siteUrl && !validUrl && <p className="text-xs text-[#FF4D4F] mt-1">请输入合法 URL</p>}
          </div>
          <div>
          ...
          <p className="text-xs text-[#A8ABB0]">站点 URL 用于分享链接与邮件内容地址拼装，变更后新生成的链接即时采用新域名。</p>
          {canUpdate && (
            <Button
              type="primary"
              loading={save.isPending}
              disabled={!basic.siteUrl || !validUrl}
              onClick={() => save.mutate({ group: 'basic', value: { siteUrl: basic.siteUrl, loginBanner: basic.loginBanner } })}
        ...
        <div className="rabbit-card p-5 space-y-4 max-w-3xl">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input value={smtp.host} onChange={(e) => setSmtp({ ...smtp, host: e.target.value })} placeholder="smtp.example.com" data-testid="input-smtp-host" />
            </div>
            <div>
            ...
            <div>
              ...
              <Input value={smtp.user} onChange={(e) => setSmtp({ ...smtp, user: e.target.value })} data-testid="input-smtp-user" />
            </div>
            <div>
            ...
            <div>
              ...
              <Radio.Group
                ...
                options={[{ value: true, label: 'SSL' }, { value: false, label: 'TLS' }]}
                data-testid="radio-smtp-ssl"
              />
            </div>
            <div>
              <span className={labelCls}>发件人</span>
              <Input value={smtp.from} onChange={(e) => setSmtp({ ...smtp, from: e.target.value })} placeholder="RabbitAITest <notify@rabbit.test>" data-testid="input-smtp-from" />
            </div>
          </div>
          <p className="text-xs text-[#A8ABB0]">SMTP 密码加密落库，读取脱敏返回 ******（保持 ****** 提交即不修改密码）。</p>
          {canUpdate && (
            <div className="flex gap-3">
              <Button loading={testSmtp.isPending} onClick={() => testSmtp.mutate()} data-testid="btn-test-smtp">
                测试连接
              </Button>
              <Button
                type="primary"
                ...
                onClick={() => save.mutate({ group: 'smtp', value: { ...smtp, port: Number(smtp.port) } })}
    ...
    },
    {
      ...
      children: (
        <div className="rabbit-card p-5 space-y-4 max-w-3xl">
          ...
          <p className="text-xs text-[#A8ABB0]">缺陷附件等全部上传场景统一读取该上限；按场景差异化上限为后续增强。</p>
          {canUpdate && (
            <Button
              type="primary"
              loading={save.isPending}
              disabled={file.maxSizeMb < 1 || file.maxSizeMb > 1024}
              onClick={() => save.mutate({ group: 'file', value: { maxSizeMb: Number(file.maxSizeMb) } })}
    ...
    },
    {
      ...
      children: (
        <div className="rabbit-card p-5 space-y-4 max-w-3xl">
          ...
          {cleanupInvalid && (
            <p className="text-xs text-[#FF4D4F]" data-testid="cleanup-invalid-hint">保留天数下限为 7 天，低于 7 天无法保存（防误配全清）</p>
          )}
          <div className="bg-[#F7F8FA] border border-[#ECEEF1] rounded-md p-3 text-[13px] text-[#646A73]" data-testid="cleanup-last-run">
            上次清理：{fmt(cleanup.lastRunAt)}（每日定时执行 03:00）· 清理 {cleanup.lastRunCount.toLocaleString()} 条（只读）
          </div>
          {canUpdate && (
            <Button
              type="primary"
              ...
              onClick={() =>
                save.mutate({
                  group: 'cleanup',
                  value: { logRetentionDays: Number(cleanup.logRetentionDays), changeLogRetentionDays: Number(cleanup.changeLogRetentionDays) },
                })
              }
