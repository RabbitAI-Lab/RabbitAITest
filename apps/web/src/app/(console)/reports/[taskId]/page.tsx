...
export default function ReportPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  const badge = {
    ...
    RUNNING: { color: "#1677ff", text: "RUNNING" },
    PENDING: { color: "#999", text: "PENDING" },
  }[status] ?? { color: "#999", text: status };
    ...
    <div className="max-w-5xl">
      <div className="rabbit-page-header">
        ...
        {data?.durationMs != null && (
          <span className="text-xs text-gray-400">{data.durationMs}ms</span>
        )}
      ...
      <div className="grid grid-cols-2 gap-4">
            ...
            {data?.request && (
              <>
                ...
                {data.request.headers.map((h) => (
                  <p key={h.key} className="text-gray-500 text-xs">
                    {h.key}: {h.value}
                  </p>
                ))}
                <p className="text-gray-500 text-xs">
                  {data.request.body ? data.request.body : "— 无 Body —"}
                </p>
              </>
            )}
          </div>
        ...
        <div className="rabbit-card" data-testid="report-response">
          ...
          {data?.response?.truncated && (
            <p className="px-3 pb-2 text-[10px] text-amber-500">响应体已截断至 256KB</p>
          )}
        </div>
      </div>
      ...
      <div className="rabbit-card mt-4" data-testid="report-asserts">
        <p className="rabbit-card-title">
          ...
          <span className="text-xs text-gray-400">
            {data?.asserts.length ?? 0} 条
            {data?.asserts.some((a) => !a.passed)
              ? ` · ${data.asserts.filter((a) => !a.passed).length} 失败`
              : ""}
          </span>
        </p>
        <Table
          ...
          columns={[
            {
              ...
              dataIndex: "kind",
              width: 140,
              render: (k: string) => (k === "status_code" ? "状态码" : "响应体 JSONPath"),
            },
            {
              title: "表达式",
              dataIndex: "path",
              width: 160,
              render: (p: string) => <span className="font-mono text-xs">{p || "—"}</span>,
            },
            {
              title: "期望",
              dataIndex: "expected",
              render: (e: string, r) => (
                <span className="font-mono text-xs">{r.op === "contains" ? `包含 ${e}` : e}</span>
              ),
            },
            {
              title: "实际",
            ...
            },
            {
              ...
              render: (p: boolean) =>
                p ? (
                  <span className="text-green-600" data-testid="assert-pass">
                    ✓ 通过
                  </span>
                ) : (
                  <span className="text-red-500 font-medium" data-testid="assert-fail">
                    ✗ 失败
                  </span>
                ),
            },
