...
export const GET = withProjectScope(async (ctx, req) => {
  try {
    ...
    return okResponse(await svc.listGroups({ scope: 'project', projectId: ctx.projectId }, new URL(req.url).searchParams.get('withMembers') === '1' ? { withMembers: '1' } : undefined));
...
export const POST = withProjectScope(async (ctx, req, _seg) => {
  try {
    ...
    return okResponse(await svc.createGroup(ctx.userId, { scope: 'project', projectId: ctx.projectId }, body), 201);
  } catch (err) { return toResponse(err); }
});
