...
## 5.5 样式加载红线（2026-09-26 事故沉淀）

1. **Tailwind 工具类必须真实可用**：新增页面后跑 `pnpm test:visual`，VISUAL 用例内置 computed-style 断言（顶栏 48px/侧栏 208px/登录页满屏），Tailwind 未安装或构建丢失会立即红灯——曾发生「类名写了但 Tailwind 未安装，录屏完全无样式」事故。
2. **禁止设置 `html { font-size }`**：Tailwind rem 类以 16px 基准对齐高保真原型；改小基准会让所有间距/高度按比例缩小（h-12 变 39px）。全局字号用 AntD `theme.token.fontSize` 控制。
