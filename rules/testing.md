...
### 1.2 覆盖单位（2026-09-26 升级：功能点 → 能力行 + 状态二态）

1. **最小覆盖单位 = 规格 §1.2 能力表每一行 P1 能力**：每行至少一条自动化断言——交互类能力（按钮/开关/弹窗/Tab/状态切换）必须落在 Playwright UI 层；纯契约/计算类可落 jmx 或 Vitest。仅"功能点主链路一条"不再视为达标（SYS-004 预置组成员管理漏测教训）。
...
0. **期望值必须溯源规格，禁止对照实现拍期望**（2026-09-26 S1 走查教训）：SYS-004-01 曾把「预置组整组只读」的错误实现断言为正确行为（三类断言齐全但期望源头错了）。写断言前先回规格 §1.2/§3 找依据；实现与规格冲突时改实现并用例先行（先写按规格应过的用例复现失败）；「预置/只读/禁用」类语义在规格评审时必须写明作用范围（如：预置组=权限点只读、成员管理照常）。
