...
describe("resolvePermissionSet 并集/禁用交集", () => {
  ...
  it("单组：授权原样生效", () => {
    expect(resolvePermissionSet([g(["PROJECT_CASE:READ"])])).toEqual(
      new Set(["PROJECT_CASE:READ"]),
    );
  });
  ...
  it("任一组禁用资源 → 整组剔除（交集语义）", () => {
    const set = resolvePermissionSet([
      g(["PROJECT_CASE:READ", "PROJECT_CASE:CREATE"]),
      g(["PROJECT_BUG:READ"], ["PROJECT_CASE"]),
    ]);
  ...
  it("禁用单点不影响同资源其它动作", () => {
    const set = resolvePermissionSet([
      g(["PROJECT_CASE:READ", "PROJECT_CASE:CREATE"], ["PROJECT_CASE:CREATE"]),
    ]);
    expect(set.has("PROJECT_CASE:READ")).toBe(true);
    expect(set.has("PROJECT_CASE:CREATE")).toBe(false);
  });
  ...
  it("禁用交集优先级高于并集（跨资源前缀匹配）", () => {
    const set = resolvePermissionSet([
      g(["PROJECT_CASE:READ", "PROJECT_BUG:READ", "PROJECT_BUG:CREATE"]),
      g([], ["PROJECT_BUG"]),
    ]);
    expect(set.has("PROJECT_CASE:READ")).toBe(true);
...
describe("buildValidator 字段引擎", () => {
  ...
  it("single_select 选项白名单", () => {
    const v = buildValidator([
      defs({ key: "sev", type: "single_select", options: { options: ["P1", "P2"] } }),
    ]);
  ...
  it("multi_select 数组且逐项白名单", () => {
    const v = buildValidator([
      defs({ key: "tags", type: "multi_select", options: { options: ["a", "b"] } }),
    ]);
  ...
  it("input 正则与长度", () => {
    const v = buildValidator([
      defs({
        key: "code",
        type: "input",
        options: { pattern: "^RB-\\d+$", minLength: 2, maxLength: 10 },
      }),
    ]);
  ...
  it("停用字段不校验（软停用语义）", () => {
    const v = buildValidator([
      defs({ key: "gone", type: "input", required: true, enabled: false }),
    ]);
...
describe("aggregateReviewResult 单人/多人", () => {
  ...
  it("任一 FAIL 即 FAIL", () => {
    expect(
      aggregateReviewResult("MULTI", reviewers, [
        { userId: "u1", result: "PASS" },
        { userId: "u2", result: "FAIL" },
      ]),
    ).toBe("FAIL");
  });
  it("全员 PASS 才 PASS", () => {
    expect(aggregateReviewResult("MULTI", reviewers, [{ userId: "u1", result: "PASS" }])).toBe(
      "PENDING",
    );
    expect(
      aggregateReviewResult("MULTI", reviewers, [
        { userId: "u1", result: "PASS" },
        { userId: "u2", result: "PASS" },
      ]),
    ).toBe("PASS");
  });
  ...
  it("单人模式=最后结果生效", () => {
    expect(
      aggregateReviewResult("SINGLE", reviewers, [
        { userId: "u1", result: "PASS" },
        { userId: "u2", result: "FAIL" },
      ]),
    ).toBe("FAIL");
