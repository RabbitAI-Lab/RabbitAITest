...
import {
  ensureSystemPresetGroups, ensureOrgPresetGroups, ensureProjectPresetGroups,
  ensureDefaultTemplates, ensureBugModule, addGroupMember,
} from '../src/presets';
...
async function main() {
  ...
  for (const [key, value] of [
    ...
    ['file', { maxSizeMb: 50 }],
    ['cleanup', { logRetentionDays: 90, changeLogRetentionDays: 90, lastRunAt: null, lastRunCount: 0 }],
  ] as const) {
    await prisma.systemParam.upsert({ where: { key }, update: {}, create: { key, value: value as object } });
  }
  ...
  await prisma.resourcePool.upsert({
    ...
    update: {},
    create: { id: '00000000-0000-0000-0000-000000000001', name: '默认资源池', type: 'NODE', isDefault: true, maxConcurrency: 4 },
  });
  ...
  if (!admin) {
    ...
    const org = await prisma.organization.create({ data: { name: '管理员组织', ownerId: admin.id }, select: { id: true } });
    await prisma.orgMember.create({ data: { orgId: org.id, userId: admin.id } });
    const project = await prisma.project.create({ data: { orgId: org.id, name: '管理项目', num: 1 }, select: { id: true } });
    ...
    for (const [scene, name] of [['case', '未规划用例'], ['api', '未规划接口']] as const) {
      await prisma.moduleNode.create({ data: { projectId: project.id, scene, name, isDefault: true } });
    }
    await initContext(org.id, project.id, admin.id);
  }
  const sysAdminGroup = await prisma.group.findFirst({ where: { scope: 'system', name: '系统管理员' }, select: { id: true } });
...
async function initContext(orgId: string, projectId: string, userId: string): Promise<void> {
  ...
  const orgAdmin = await prisma.group.findFirst({ where: { scope: 'org', orgId, name: '组织管理员' }, select: { id: true } });
  if (orgAdmin) await addGroupMember(prisma, orgAdmin.id, userId);
  const projAdmin = await prisma.group.findFirst({ where: { scope: 'project', projectId, name: '项目管理员' }, select: { id: true } });
