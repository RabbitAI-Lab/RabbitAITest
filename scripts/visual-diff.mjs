...
const manifest = [
  { name: '登录页', prototype: 'docs/design/SYS-001-registration-login/index.html', shot: 'tests/visual/snapshots/login.png' },
  { name: '顶栏与工作台（项目切换）', prototype: 'docs/design/SYS-003-org-project-init/index.html', shot: 'tests/visual/snapshots/dashboard.png' },
  { name: '用例列表', prototype: 'docs/design/CASE-001-case-crud/list.html', shot: 'tests/visual/snapshots/case-list.png' },
  { name: '新建用例表单', prototype: 'docs/design/CASE-001-case-crud/form.html', shot: 'tests/visual/snapshots/case-form.png' },
  { name: '调试台', prototype: 'docs/design/API-001-http-debug/index.html', shot: 'tests/visual/snapshots/debug.png' },
  { name: '执行报告', prototype: 'docs/design/RPT-001-execution-report-mvp/index.html', shot: 'tests/visual/snapshots/report.png' },
];
...
async function renderPrototypes(items) {
  ...
  for (const it of items) {
    ...
    await page.goto('file://' + path.join(ROOT, it.prototype), { waitUntil: 'networkidle', timeout: 30000 }).catch(() => undefined);
      ...
      messages: [
        {
          ...
          content: [
            ...
            { type: 'image_url', image_url: { url: `data:image/png;base64,${b64(path.join(ROOT, it.shot))}` } },
          ],
        },
      ],
    }),
...
async function main() {
  ...
  if (missing.length > 0) {
    console.error('缺少实现截图，先运行：pnpm test:visual\n  ' + missing.map((m) => m.shot).join('\n  '));
    process.exit(2);
  }
  if (!API_KEY) {
  ...
  if (ENFORCE) {
    ...
    if (bad.length > 0) {
      console.error(`visual-diff: ${bad.length} 项低于阈值 ${THRESHOLD}（${bad.map((b) => b.name).join('、')}）`);
      process.exit(1);
    }
  }
