...
for (const file of walk(API_ROOT)) {
  ...
  for (const method of METHOD_ORDER) {
    ...
    const permMatch = src.match(
      new RegExp(`(?:withSystemPerm\\('|requirePerm\\(')([A-Z_]+:[A-Z_]+)`),
    );
...
const doc = {
  ...
  info: {
    ...
    version: "0.2.0",
    description:
      "由 scripts/gen-openapi.mjs 从 Route Handlers 生成（路径/方法/权限点单一来源）；请求/响应契约见 packages/shared zod schema。",
  },
...
const offenders = [...handPaths]
  ...
  .filter((p) => {
    ...
    for (const r of registrySkel) {
      if (r === sk || r.startsWith(sk) || sk.startsWith(r + "-") || sk.startsWith(r + "/"))
        return false;
    }
    return true;
  });
...
if (CHECK) {
  ...
  if (current !== json) {
    console.error(
      "[gen-openapi] 快照过期：路由与 openapi.json 不一致。请运行 node scripts/gen-openapi.mjs 并提交。",
    );
    process.exit(1);
  }
  console.log(
    `[gen-openapi] check ok：${Object.keys(paths).length} paths 快照一致；s1.ts 手写路径 ${handPaths.size} 条全部在册`,
  );
} else {
  mkdirSync(path.dirname(SNAPSHOT), { recursive: true });
  writeFileSync(SNAPSHOT, json);
  console.log(
    `[gen-openapi] 生成 ${Object.keys(paths).length} paths → ${path.relative(ROOT, SNAPSHOT)}；s1.ts 手写路径 ${handPaths.size} 条审计通过`,
  );
}
