...
test("CASE-003-01 详情七 Tab 与依赖双向", async ({
  authedPage,
  page,
  request,
  ...
  await expect(page.getByTestId("case-table")).toBeVisible();
  await page
    .getByRole("row", { name: new RegExp(nameB) })
    .getByRole("link", { name: /^C-\d{4,}$/ })
    .click();
  ...
  const postComment = () =>
    page.waitForResponse(
      (r) =>
        r.request().method() === "POST" && /\/api\/v1\/projects\/[^/]+\/comments\?/.test(r.url()),
    );
...
test("CASE-003-02 Markdown 注入转义", async ({
  authedPage,
  page,
  request,
  expectNoConsoleErrors,
