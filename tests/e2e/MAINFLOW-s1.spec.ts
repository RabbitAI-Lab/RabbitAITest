...
test("MAINFLOW-s1 Sprint1 主链路（模块树→用例→评审→计划→执行→缺陷→看板）", async ({
  authedPage,
  page,
  request,
  ...
  const mods = (
    (await modsRes.json()) as {
      code: number;
      data: { items: { id: string; name: string; children: { id: string; name: string }[] }[] };
    }
  ).data.items;
  ...
  for (const body of [
    {
      name: caseAName,
      precondition: "已注册账号",
      steps: [{ desc: "输入正确账号密码点击登录", expect: "跳转工作台" }],
    },
    {
      name: caseBName,
    ...
    const r = await request.post(`/api/v1/projects/${projectId}/cases`, {
      data: { ...body, moduleId: childId },
    });
  ...
  await reviewLinkModal
    .getByRole("row", { name: new RegExp(caseAName) })
    .locator('input[type="checkbox"]')
    .check();
  await reviewLinkModal
    .getByRole("row", { name: new RegExp(caseBName) })
    .locator('input[type="checkbox"]')
    .check();
  ...
  const execBPayload = execBRawRes.request().postDataJSON() as {
    status: string;
