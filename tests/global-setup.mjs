...
function log(m) { console.log(`\x1b[36m[e2e-setup]\x1b[0m ${m}`); }
...
async function ensureRedis() {
  ...
  try {
    ...
    spawnSync('docker', ['run', '-d', '--name', 'rabbit-e2e-redis', '-p', '6381:6379', 'redis:7-alpine'], { stdio: 'ignore' });
...
export default async function globalSetup() {
  ...
  writeFileSync(path.join(root, 'tests', '.e2e.env'),
    `DATABASE_URL=${env.DATABASE_URL}\nREDIS_URL=${env.REDIS_URL}\nWEB_URL=${env.WEB_URL}\nSESSION_SECRET=${env.SESSION_SECRET}\nINTERNAL_TOKEN=${env.INTERNAL_TOKEN}\nPORT=3100\nSESSION_COOKIE_SECURE=false\n`);
  log('migrate deploy + seed …');
  const migrate = spawnSync('pnpm', ['--filter', '@rabbit/db', 'migrate-deploy'], { stdio: 'inherit', env, cwd: root });
